...
Qed.    
  ...
  Lemma wf_ctx_shift_ctx_strengthen : forall c a G, wf_ctx (a + c) (shift_ctx 0 a G) -> wf_ctx c G.
  Proof.
    unfold wf_ctx.
    intros.
    replace c with (c + 0) by lia.
    eapply wf_typ_shift_strengthen with (c := a).
    replace (c + a + 0) with (a + c) by lia.
    apply H.
...
(* wf_typ_shift : ∀ (a b c : nat) (t : typ), a + b ⊢ t wf → a + c + b ⊢ shift_typ b c t wf *)
  ...
  Lemma shift_ctx_le_swap: ∀ (c1 c2 b1 b2 : nat) G, c2 ≤ c1 → shift_ctx c2 b2 (shift_ctx c1 b1 G) = shift_ctx (c1 + b2) b1 (shift_ctx c2 b2 G).
  Proof.
    intros c1 c2 b1 b2 G. revert c1 c2 b1 b2.
    induction G; intros; simpl; auto.
    rewrite shift_typ_le_swap; try lia. rewrite IHG; try lia. reflexivity.
  Qed.
...
| pfn_exists : forall n c G D1 D u,
    n ⊣ (1 + c) , (shift_ctx 0 1 G) , (shift_ctx 0 1 D1) ++ [u] ⊢pf ->
                                  D ≡[P] D1 ++ [ [exists] u ] ->
                                  (S n) ⊣ c , G , D ⊢pf
  where
...
Lemma pfn_perm_rel_iff_exists: forall c G1 G2 D1 D2,
  ...
  Lemma wf_typ_shift_le : forall a b c t,
      a <= c ->
      wf_typ (c + b) (shift_typ a b t) ->
      wf_typ c t.
  Proof.
    intros a b c t. revert a b c.
    induction t; intros; simpl in *; subst; auto.
    - destruct (Nat.ltb_spec x a).
      + constructor; lia.
      + inversion H0; subst; constructor; lia.
    - inversion H0; constructor; eauto.
    - inversion H0; constructor; eauto.
    - inversion H0; constructor; eauto.
    - inversion H0; constructor; eauto.
    - inversion H0; subst. constructor. eapply IHt.
      2: {replace (1 + (c + b)) with (1 + c + b) in H3 by lia. eassumption. }
      lia.
    - inversion H0; subst. constructor. eapply IHt.
      2: {replace (1 + (c + b)) with (1 + c + b) in H3 by lia. eassumption. }
      lia.
  ...
  Lemma wf_ctx_shift_le: ∀ (a b c : nat) (G : list typ),
      a <= c ->
      wf_ctx (c + b) (shift_ctx a b G) → wf_ctx c G.
  Proof.
    intros a b c G. revert a b c.
    induction G; intros; subst.
    - unfold wf_ctx; intros; inversion H1.
    - replace (a :: G) with ([a] ++ G) in * by auto.
      rewrite shift_ctx_app in H0. apply wf_ctx_app in H0 as (HW1 & HW2).
      apply wf_ctx_app ; split; eauto.
      eapply wf_ctx_single. eapply wf_typ_shift_le.
      + eassumption.
      + eapply wf_ctx_single in HW1. eassumption.
  Qed.
...
Proof.
  unfold wf_ctx.
  intros.
  unfold shift_ctx in H0.
  apply in_map_iff in H0.
  destruct H0 as [u' [HEQ HIN]].
  subst.
  replace (c + b) with (c + b + 0) by lia.
  apply wf_typ_shift.
...
Lemma wf_typ_shift_le_inv : forall a b c t,
    a <= c ->
    wf_typ c t ->
    wf_typ (c + b) (shift_typ a b t).
Proof.
  ...
  - inversion H0; subst. constructor.
    replace (1 + (c + b)) with ((1 + c) + b) by lia.
    eapply IHt; try lia; assumption.
  - inversion H0; subst. constructor.
    replace (1 + (c + b)) with ((1 + c) + b) by lia.
    eapply IHt; try lia; assumption.
Qed.

Lemma wf_ctx_shift_le_inv: ∀ (a b c : nat) (G : list typ),
    a <= c ->
    wf_ctx c G ->
    wf_ctx (c + b) (shift_ctx a b G).
Proof.
  intros a b c G. revert a b c.
  induction G; intros; subst.
  - unfold wf_ctx; intros; inversion H1.
  - replace (a :: G) with ([a] ++ G) in * by auto.
    rewrite shift_ctx_app. apply wf_ctx_app in H0 as (HW1 & HW2).
    apply wf_ctx_app ; split; eauto.
    eapply wf_ctx_single. eapply wf_typ_shift_le_inv.
    + eassumption.
    + eapply wf_ctx_single in HW1. eassumption.
Qed.

Corollary wf_typ_shift_le_iff : forall a b c t,
    a <= c ->
    wf_typ c t <-> wf_typ (c + b) (shift_typ a b t).
Proof.
  intros; split; intros.
  - apply wf_typ_shift_le_inv; eauto.
  - eapply wf_typ_shift_le; eauto.
Qed.

Corollary wf_ctx_shift_le_iff : forall a b c G,
    a <= c ->
    wf_ctx c G <-> wf_ctx (c + b) (shift_ctx a b G).
Proof.
  intros; split; intros.
  - eapply wf_ctx_shift_le_inv; eauto.
  - eapply wf_ctx_shift_le; eauto.
Qed.
...
Proof.
  ...
  - eapply pfn_forall; try eassumption.
    eapply IHHP; eassumption.
  - eapply pfn_exists.
    + apply IHHP with (G2 := shift_ctx 0 1 G2).
      ++ 
        apply (Permutation_rel_shift_ctx 1 0) in HG.
        rewrite shift_ctx_app in HG.
        assumption.
      ++ replace (1 + c) with (c + 1). apply wf_shift_ctx0; assumption. lia.
    + assumption.
(* wf_ctx_shift: ∀ (a b c : nat) (G : ctx), wf_ctx (a + b) G → wf_ctx (a + c + b) (shift_ctx b c G) *)
Qed.
...
Lemma pfn_absorb_append : forall n D D1 D2 G c, wf_ctx c (G ++ D2) -> D ≡[P] D1 ++ D2 -> n ⊣ c, (G ++ D2), D ⊢pf -> (n + length D2) ⊣ c, (G ++ D2), D1 ⊢pf.
  ...
  Definition cut_admissibility_t n m c G1 G1' G2 G2' G3 D1 D1' D2 D2' A1 A2 A3 A4 b :=
    D1 ≡[P] D1' ++ shift_ctx 0 b A1 ->
    D2 ≡[P] D2' ++ shift_ctx 0 b A2 ->
    G1 ≡[P] G1' ++ shift_ctx 0 b A3 ->
    G2 ≡[P] G2' ++ shift_ctx 0 b A4 ->
    n ⊣ c, G1, D1 ⊢pf ->
    m ⊣ c, G2, D2 ⊢pf ->
               n + m ⊣ c, G3, D1' ++ D2' ⊢pf.

  Definition cut_admissibility_norm n m u b := forall c G D1 D1' D2 D2', cut_admissibility_t n m c G G G G G D1 D1' D2 D2' [u] [dual u] [] [] b.
  
  Lemma test : forall n m u b, cut_admissibility_norm n m u b.
  Proof.
    unfold cut_admissibility_norm. unfold cut_admissibility_t. Abort.


  Definition cut_admissibility_t_exists n m c G1 G1' G2 G2' G3 D1 D1' D2 D2' A1 A2 A3 A4 b :=
    D1 ≡[P] D1' ++ shift_ctx 0 b A1 ->
    D2 ≡[P] D2' ++ shift_ctx 0 b A2 ->
    G1 ≡[P] G1' ++ shift_ctx 0 b A3 ->
    G2 ≡[P] G2' ++ shift_ctx 0 b A4 ->
  ...
  Definition cut_admissibility_bang n m u b := forall c G1 G2 D1 D2, cut_admissibility_t_exists n m c G1 G1 G2 G1 G1 D1 [] D2 D2 [u] [] [] [dual u] b.
  Lemma test : forall n m u b, cut_admissibility_bang n m u b.
  Proof. unfold cut_admissibility_bang. unfold cut_admissibility_t. Abort.

  Definition cut_admissibility_ques n m u b := forall c G1 G2 D1 D2, cut_admissibility_t_exists n m c G1 G2 G2 G2 G2 D1 D1 D2 [] [] [dual u] [u] [] b.
  Lemma test : forall n m u b, cut_admissibility_ques n m u b.
  Proof. unfold cut_admissibility_ques. unfold cut_admissibility_t. Abort.

  Lemma cut_admissibility_imp_norm :
    (forall n m u b,
        cut_admissibility_norm n m u b
        /\ ((forall n' m' b, cut_admissibility_norm n' m' u b) -> cut_admissibility_bang n m u b)
        /\ ((forall n' m' b, cut_admissibility_norm n' m' u b) -> cut_admissibility_ques n m u b)) ->
    (forall n m u b, cut_admissibility_norm n m u b).
  Proof.
    intros. specialize (H n m u b). intuition.
  Qed.
  ...
  Lemma cut_admissibility_imp_bang :
    (forall n m u b,
        ...
        /\ ((forall n' m' b, cut_admissibility_norm n' m' u b) -> cut_admissibility_bang n m u b)
        /\ ((forall n' m' b, cut_admissibility_norm n' m' u b) -> cut_admissibility_ques n m u b)) ->
    (forall n m u b, cut_admissibility_bang n m u b).
  Proof.
    intros. pose proof H as H'.
    specialize (H' n m u b) as (_ & H' & _).
  ...
  Lemma cut_admissibility_imp_ques :
    (forall n m u b,
        ...
        /\ ((forall n' m' b, cut_admissibility_norm n' m' u b) -> cut_admissibility_bang n m u b)
        /\ ((forall n' m' b, cut_admissibility_norm n' m' u b) -> cut_admissibility_ques n m u b)) ->
    (forall n m u b, cut_admissibility_ques n m u b).
  Proof.
    intros. pose proof H as H'.
    specialize (H' n m u b) as (_ & _ & H').
    pose proof (cut_admissibility_imp_norm H).
    apply H'. intros.
    apply H0.
  Qed.
  ...
  Lemma cut_admissibility_imp_norm2 :
    forall u,
      (forall n m b,
          ...
          /\ ((forall n' m' b, cut_admissibility_norm n' m' u b) -> cut_admissibility_bang n m u b)
          /\ ((forall n' m' b, cut_admissibility_norm n' m' u b) -> cut_admissibility_ques n m u b)) ->
      (forall n m b, cut_admissibility_norm n m u b).
  Proof.
    intros. specialize (H n m b). intuition.
  Qed.
  ...
  Lemma cut_admissibility_imp_bang2 :
    forall u,
      (forall n m b,
          ...
          /\ ((forall n' m' b, cut_admissibility_norm n' m' u b) -> cut_admissibility_bang n m u b)
          /\ ((forall n' m' b, cut_admissibility_norm n' m' u b) -> cut_admissibility_ques n m u b)) ->
      (forall n m b, cut_admissibility_bang n m u b).
  Proof.
    intros. pose proof H as H'.
    specialize (H' n m b) as (_ & H' & _).
  ...
  Lemma cut_admissibility_imp_ques2 :
    forall u,
      (forall n m b,
          ...
          /\ ((forall n' m' b, cut_admissibility_norm n' m' u b) -> cut_admissibility_bang n m u b)
          /\ ((forall n' m' b, cut_admissibility_norm n' m' u b) -> cut_admissibility_ques n m u b)) ->
      (forall n m b, cut_admissibility_ques n m u b).
  Proof.
    intros. pose proof H as H'.
    specialize (H' n m b) as (_ & _ & H').
    pose proof (cut_admissibility_imp_norm2 u H).
    auto.
  Qed.

  
  (* ∃ k : nat, k ≤ S (m `max` n) ∧ (k ⊣ c, G, D1' ++ D2' ⊢pf) *)
  (* Lemma pfn_perm_rel_iff_le_exists : *)
  (*   ∀ m (c : nat) (G1 G2 D1 D2 : list typ), *)
  (*     G1 ≡[ P] G2 → *)
  (*     D1 ≡[ P] D2 → *)
  (*     (∃ k1 : nat, k1 <= m /\ (k1 ⊣ c, G1, D1 ⊢pf)) ↔ ∃ k2 : nat, k2 <= m /\ (k2 ⊣ c, G2, D2 ⊢pf). *)
  (* Proof. *)
  (*   intros; split; intros. *)
  (*   - destruct H1 as (k1 & H'1 & H'2). *)
  (*     exists k1; split; intuition. *)
  (*     eapply pfn_perm_rel; try eassumption. *)
  (*   - destruct H1 as (k2 & H'1 & H'2). *)
  (*     exists k2; split; intuition. *)
  (*     eapply pfn_perm_rel_iff; try eassumption. *)
  (* Qed. *)
  
  Lemma cut_admissibility_norm_dual : forall n m u b, cut_admissibility_norm n m u b <-> cut_admissibility_norm m n (dual u) b.
  Proof.
    intros. split; unfold cut_admissibility_norm, cut_admissibility_t; intros.
    -
      eapply pfn_perm_rel. reflexivity. apply Permutation_rel_exchange.
      (* replace (S (m `max` n)) with (S (n `max` m)) by lia. *)
      (* replace (m `max` n) with (n `max` m) by lia. *)
      rewrite dual_involutive in H1.
      specialize (H _ _ _ _ _ _ H1 H0 H3 H2 H5 H4). intuition.
      replace (m + n) with (n + m) in * by lia; intuition.
    (* replace (S (m `max` n)) with (S (n `max` m)) by lia. intuition. *)
      (* replace (m + n) with (n + m) by lia. intuition. *)
      (* eapply H. *)
    ...
    -
      eapply pfn_perm_rel. reflexivity. apply Permutation_rel_exchange.
      replace u with (dual (dual u)) in H0 by apply dual_involutive.
      specialize (H _ _ _ _ _ _ H1 H0 H3 H2 H5 H4).
      replace (m + n) with (n + m) in * by lia; intuition.
      (* replace (m `max` n) with (n `max` m) in * by lia. intuition. *)
      (* replace (S (m `max` n)) with (S (n `max` m)) in * by lia. intuition. *)
      (* replace (m + n) with (n + m) in * by lia. intuition. *)
      (* eapply pfn_perm_rel_iff_exists. reflexivity. apply Permutation_rel_exchange. *)
      (* (* replace (S (n `max` m)) with (S (m `max` n)) by lia. *) *)
      (* eapply H. *)
      (* + apply H1. *)
  ...
  Lemma cut_admissibility_bang_dual : forall n m u b, cut_admissibility_bang n m u b <-> cut_admissibility_ques m n (dual u) b.
  Proof.
    intros. split; unfold cut_admissibility_bang, cut_admissibility_ques, cut_admissibility_t, cut_admissibility_t_exists; intros.
    -
      eapply pfn_perm_rel_iff_exists. reflexivity. apply Permutation_rel_exchange.
      rewrite dual_involutive in H1.
      specialize (H _ _ _ _ _ H1 H0 H3 H2 H5 H4).
      replace (m + n) with (n + m) in * by lia; intuition.
      (* replace (m `max` n) with (n `max` m) in * by lia. intuition. *)
      (* replace (S (m `max` n)) with (S (n `max` m)) in * by lia. intuition. *)
      (* replace (m + n) with (n + m) in * by lia. intuition. *)
  ...
  Lemma cut_admissibility_ques_dual : forall n m u b, cut_admissibility_ques n m u b <-> cut_admissibility_bang m n (dual u) b.
    Proof.
    intros. split; unfold cut_admissibility_ques, cut_admissibility_bang, cut_admissibility_t, cut_admissibility_t_exists; intros.
    -
      eapply pfn_perm_rel_iff_exists. reflexivity. apply Permutation_rel_exchange.
      rewrite dual_involutive in H3.
      specialize (H _ _ _ _ _ H1 H0 H3 H2 H5 H4).
      replace (m + n) with (n + m) in * by lia; intuition.
      (* replace (m `max` n) with (n `max` m) in * by lia. intuition. *)
      (* replace (S (m `max` n)) with (S (n `max` m)) in * by lia. intuition. *)
    ...
    -
      eapply pfn_perm_rel_iff_exists. reflexivity. apply Permutation_rel_exchange.
      replace u with (dual (dual u)) in H2 by apply dual_involutive.
      specialize (H _ _ _ _ _ H1 H0 H3 H2 H5 H4).
      replace (m + n) with (n + m) in * by lia; intuition.
      (* replace (m `max` n) with (n `max` m) in * by lia. intuition. *)
      (* replace (S (m `max` n)) with (S (n `max` m)) in * by lia. intuition. *)
  ...
  Lemma cut_admissibility_dual :
    forall u,
      (forall n m b, 
          cut_admissibility_norm n m u b
          ∧ ((∀ n' m' b : nat, cut_admissibility_norm n' m' u b) → cut_admissibility_bang n m u b)
          ∧ ((∀ n' m' b : nat, cut_admissibility_norm n' m' u b) → cut_admissibility_ques n m u b))
      <-> (forall n m b, cut_admissibility_norm n m (dual u) b
                ∧ ((∀ n' m' b : nat, cut_admissibility_norm n' m' (dual u) b) → cut_admissibility_bang n m (dual u) b)
                ∧ ((∀ n' m' b : nat, cut_admissibility_norm n' m' (dual u) b) → cut_admissibility_ques n m (dual u) b)).
  Proof.
    ...
    Hint Resolve <- cut_admissibility_ques_dual : core.
    intros. split; intros; specialize (H m n b) as (H'1 & H'2 & H'3); repeat split; eauto.
  Qed.

  Ltac unfold_admissibilityH H :=
    unfold cut_admissibility_bang, cut_admissibility_ques, cut_admissibility_norm, cut_admissibility_t, cut_admissibility_t_exists in H.
  
  Ltac unfold_admissibility :=
    unfold cut_admissibility_bang, cut_admissibility_ques, cut_admissibility_norm, cut_admissibility_t, cut_admissibility_t_exists.
  ...
  Lemma shift_typ0 : forall a t,
      shift_typ a 0 t = t.
  Proof.
    intros a t. revert a. induction t; intros; simpl; auto.
    - destruct (Nat.ltb_spec x a); auto.
    - rewrite IHt1, IHt2. auto.
    - rewrite IHt1, IHt2. auto.
    - rewrite IHt. auto.
    - rewrite IHt. auto.
  ...
  Lemma shift_ctx0 : forall a G,
      shift_ctx a 0 G = G.
  Proof.
    intros a G. revert a. induction G; intros; simpl; auto.
    rewrite IHG, shift_typ0. auto.
  Qed.

  Lemma pfn_shift_inv: forall a b c n G D,
      a <= c ->
      n ⊣ c, G, D ⊢pf ->
  ...
  Proof.
    ...
    intros a b c n G D HLE HG. revert a b HLE.
    induction HG; intros.
    - rewrite shift_ctx_app. simpl. rewrite dual_shift_typ_comm.
    ...
    - eapply pfn_absorb.
      ...
      apply IHHG; assumption. (* intros. apply IHY; auto. intros. apply IHWF; auto. *)
    - simpl. apply pfn_bot.
      eapply wf_ctx_shift_gen. auto.
    - eapply pfn_one.
      2: {rewrite H. shift_ctx_eq. }
      eapply IHHG; eauto. (* intros. apply IHY; auto. intros. apply IHWF; auto. *)
    - eapply pfn_tensor.
      2: {rewrite H. shift_ctx_eq. }
      ...
      eapply IHHG; eauto. (* intros. apply IHY; auto. intros. apply IHWF; auto. *)
    - eapply pfn_par.
      3: {rewrite H. shift_ctx_eq. }
      + replace (shift_ctx a b D1 ++ [shift_typ a b t]) with (shift_ctx a b (D1 ++ [t])).
        2: {shift_ctx_eq. }
        eapply IHHG1; eauto. (* intros. apply IHY. lia. intros. apply IHWF; auto. lia. *)
      + replace (shift_ctx a b D2 ++ [shift_typ a b u]) with (shift_ctx a b (D2 ++ [u])).
        2: {shift_ctx_eq. }
        eapply IHHG2; eauto. (* intros. apply IHY. lia. intros. apply IHWF; auto. lia. *)
    - eapply pfn_bang.
      2: {rewrite H. shift_ctx_eq. }
      replace (shift_ctx a b G ++ [shift_typ a b t]) with (shift_ctx a b (G ++ [t])).
      2: {shift_ctx_eq. }
      eapply IHHG; eauto. (* intros. apply IHY. lia. intros. apply IHWF; auto; lia. *)
    ...
    - eapply pfn_forall with (u := shift_typ a b u).
      3: {rewrite H0. shift_ctx_eq. }
      + eapply wf_typ_shift_gen. assumption.
      + rewrite typ_subst_shift_typ_comm_gen. 2: {lia. }
      replace (shift_ctx a b D1 ++ [shift_typ a b (typ_subst 0 u t)]) with (shift_ctx a b (D1 ++ [typ_subst 0 u t])) by shift_ctx_eq.
      eapply IHHG; eauto. (* intros. apply IHY. lia. intros. apply IHWF; auto; lia. *)
    - eapply pfn_exists.
      2: {rewrite H. shift_ctx_eq. }
      replace (shift_ctx 0 1 (shift_ctx a b G)) with (shift_ctx (a + 1) b (shift_ctx 0 1 G)).
      2: {rewrite <- shift_ctx_le_swap; try lia. reflexivity. }
      replace (shift_ctx 0 1 (shift_ctx a b D1)) with (shift_ctx (a + 1) b (shift_ctx 0 1 D1)).
      2: {rewrite <- shift_ctx_le_swap; try lia. reflexivity. }
      replace ([shift_typ (a + 1) b u]) with (shift_ctx (a + 1) b [u]) by auto.
      rewrite <- shift_ctx_app.
      replace (1 + (c + b)) with ((1 + c) + b) by lia.
      eapply IHHG; lia.
  ...
  (*     apply IHWF. lia. apply H. lia. *)
  (*   - eapply pfn_shift. *)
  (*     + assert (a <= c + b0) by lia. eassumption. *)
  (*     +  *)
  (* Qed. *)
  ...
  Lemma pfn_shift_exists_subst_base : forall n a b c G G' D D' p y,
      (c + b) ⊢ t_base p y wf ->
      G' ≡[P] shift_ctx a b G ->
      D' ≡[P] shift_ctx a b D ++ [t_base p y] ->
      n ⊣ (c + b), G', D' ⊢pf ->
      exists u',
        c ⊢ u' wf /\
          (n ⊣ (c + b), G', shift_ctx a b D ++ [u'] ⊢pf).
  Proof.
    intros.
    exists (t_base p y); intuition.
    eapply pfn_perm_rel_iff. reflexivity. symmetry. eassumption. eassumption.
  Qed.

  

  (* Lemma pfn_shift_exists_subst_var : forall n a b c G G' D D' p y, *)
  (*     (c + b) ⊢ t_var p y wf -> *)
  (*     b >= 1 -> a < c -> *)
  (*     G' ≡[P] shift_ctx a b G -> *)
  (*     D' ≡[P] shift_ctx a b D ++ [t_var p y] -> *)
  (*     n ⊣ (c + b), G', D' ⊢pf -> *)
  (*     exists u', *)
  (*       c ⊢ u' wf /\ *)
  (*         (n ⊣ (c + b), G', shift_ctx a b D ++ [u'] ⊢pf). *)
  (* Proof. *)
  (*   intros n a b c G G' D D' p y HWFC HLE1 HLE2 HPG HPD HG. destruct (Nat.ltb_spec y a). *)
  (*   {exists (t_var p y); intuition. *)
  (*    - constructor; lia. *)
  (*    - eapply pfn_perm_rel. reflexivity. eassumption. eassumption. *)
  (*   } *)
  (*   destruct (Nat.ltb_spec y (a + b)). *)
  (*   - remember (c + b) as c'. revert a b c G D p y HWFC HLE1 HLE2 HPG HPD Heqc' H H0. *)
  (*     induction HG; intros. *)
  (*     + eapply Permutation_rel_split2 in HPD. destruct HPD as [[E1 [HPE1 HPE2]] | [E2 [HPE1 HPE2]]]. *)
  (*       * symmetry in HPE1. eapply Permutation_rel_singleton_nil in HPE1 as (-> & HPE1). subst. *)
  (*         simpl in HPE2. symmetry in HPE2. eapply Permutation_rel_singleton in HPE2. *)
  (*         do 2 (destruct D; try discriminate). *)
  (*         destruct t; try discriminate. simpl in HPE2. *)
  (*         destruct (Nat.ltb_spec x a); injection HPE2; intros; lia. *)
  (*       * symmetry in HPE1. *)

  Inductive gap_typ : nat -> nat -> typ -> Prop :=
  | gap_t_base : forall a b p (bt:base_type), gap_typ a b (t_base p bt)
  | gap_t_var : forall a b p (x:nat) (LT: x < a \/ a + b <= x), gap_typ a b (t_var p x)
  | gap_t_tensor : forall a b (t1 t2 : typ), gap_typ a b t1 -> gap_typ a b t2 -> gap_typ a b (t1 ⊗ t2)
  | gap_t_par : forall a b (t1 t2 : typ), gap_typ a b t1 -> gap_typ a b t2 -> gap_typ a b (t1 ∥ t2)
  | gap_t_bang : forall a b t, gap_typ a b t -> gap_typ a b ([!]t)
  | gap_t_ques : forall a b t, gap_typ a b t -> gap_typ a b ([?]t)
  | gap_t_forall :
    forall a b t, gap_typ (1 + a) b t -> gap_typ a b ([forall] t)
  (* forall c t, wf_typ c t -> wf_typ c ([forall] t) *)
  | gap_t_exists : forall a b t, gap_typ (1 + a) b t -> gap_typ a b ([exists] t)
  .                                               

  Lemma typ_shift_gap : forall a b t t', t' = shift_typ a b t -> gap_typ a b t'.
  Proof.
    intros a b t t' HS. revert a b t' HS.
    induction t; intros; simpl in HS.
    - rewrite HS. constructor.
    - destruct (Nat.ltb_spec x a); rewrite HS; constructor; lia.
    - rewrite HS; constructor; eauto.    
    - rewrite HS; constructor; eauto.
    - rewrite HS; constructor; eauto.
    - rewrite HS; constructor; eauto.
    - rewrite HS; constructor; eauto.
      eapply IHt. replace (a + 1) with (1 + a) by lia. auto.
    - rewrite HS; constructor; eauto.
      eapply IHt. replace (a + 1) with (1 + a) by lia. auto.
  Qed.

  Definition gap_ctx a b D := forall t, In t D -> (gap_typ a b t).

  Fixpoint drop_typ b c (t:typ) :=
    match t with
    | t_base p n => t_base p n
    | t_var p x => if Nat.ltb x b then t_var p x else
                    (* t_var p (x - c) *)
                    if Nat.ltb x c then t_base p b_unit else
                      t_var p (x - c)
    | t1 ⊗ t2 => (drop_typ b c t1) ⊗ (drop_typ b c t2)
    | t1 ∥ t2 => (drop_typ b c t1) ∥ (drop_typ b c t2)
    | [!]t => [!](drop_typ b c t)
    | [?]t => [?](drop_typ b c t)
    | [forall]t => [forall](drop_typ (b + 1) c t)
    | [exists]t => [exists](drop_typ (b + 1) c t)
    end.

  Definition drop_ctx (b c:nat) := List.map (drop_typ b c).

  Lemma drop_typ0 : forall a t, drop_typ a 0 t = t.
  Proof.
    intros a t. revert a.
    induction t; intros; simpl in *; auto.
    - destruct (Nat.ltb_spec x a); auto. replace (x - 0) with x by lia. auto.
    - rewrite IHt1, IHt2. reflexivity.
    - rewrite IHt1, IHt2. reflexivity.
    - rewrite IHt. reflexivity.
    - rewrite IHt. reflexivity.
  ...
  Lemma drop_ctx0 : forall a D, drop_ctx a 0 D = D.
  Proof.
    intros a D. revert a.
    induction D; intros; simpl; auto.
    rewrite IHD. rewrite drop_typ0. reflexivity.
  Qed.

  Lemma drop_ctx_app : forall a b D1 D2,
      drop_ctx a b (D1 ++ D2) = drop_ctx a b D1 ++ drop_ctx a b D2.
  Proof.
    intros. 
    apply map_app.
  Qed.

  Lemma drop_ctx_cons : forall a b t D,
      drop_ctx a b (t :: D) = drop_ctx a b [t] ++ drop_ctx a b D.
  Proof.
    intros.
    apply map_cons.
  Qed.

  Lemma dual_drop_typ_comm : forall t b c, drop_typ b c (dual t) = dual (drop_typ b c t).
  Proof.
    intros t. induction t; intros; simpl; auto.
    - destruct (x <? b); auto. destruct (x <? c); auto.
    - rewrite IHt1, IHt2. auto.
    - rewrite IHt1, IHt2. auto.
    - rewrite IHt. auto.
    - rewrite IHt. auto.
  ...
  Hint Constructors gap_typ : core.

  Lemma gap_typ_shift : forall a b t,
      gap_typ a b (shift_typ a b t).
  Proof.
    intros a b t. revert a b.
    induction t; intros.
    - constructor.
    - simpl. destruct (Nat.ltb_spec x a).
      + constructor; lia.
      + constructor; lia.
    - constructor; eauto.
    - constructor; eauto.
  ...
  Lemma gap_shift_eq : forall a b t,
      gap_typ a b t <-> exists t', shift_typ a b t' = t.
  Proof.
    intros; split; intros.
    - revert a b H. induction t; intros.
      + exists (t_base p b); reflexivity.
      + inversion H. destruct LT; subst.
        * exists (t_var p x). simpl. apply Nat.ltb_lt in H2. rewrite H2; auto.
        * exists (t_var p (x - b)). simpl.
          assert (not (x - b < a)) by lia. apply Nat.ltb_nlt in H0. rewrite H0.
          replace (b + (x - b)) with (x) by lia. auto.
      + inversion H; subst. apply IHt1 in H4 as (u1 & H4). apply IHt2 in H5 as (u2 & H5).
        exists (u1 ⊗ u2); simpl; subst; eauto.
      + inversion H; subst. apply IHt1 in H4 as (u1 & H4). apply IHt2 in H5 as (u2 & H5).
        exists (t_par u1 u2); simpl; subst; eauto.
      + inversion H; subst. apply IHt in H3 as (u & H3).
        exists ([!]u); simpl; subst; eauto.
      + inversion H; subst. apply IHt in H3 as (u & H3).
        exists ([?]u); simpl; subst; eauto.
      + inversion H; subst. apply IHt in H3 as (u & H3).
        exists ([forall]u). simpl. replace (a + 1) with (1 + a) by lia. simpl; subst; eauto.
      + inversion H; subst. apply IHt in H3 as (u & H3).
  ...
  Proof.
    ...
    intros.
    subst.
    eapply Permutation_rel_drop_ctx.
    assumption.
  Qed.

  Lemma gap_ctx_nil : forall a b, gap_ctx a b [].
  Proof.
    intros. unfold gap_ctx; intros. inversion H.
  Qed.

  Lemma gap_ctx_app : forall a b D1 D2,
      gap_ctx a b (D1 ++ D2) <-> gap_ctx a b D1 /\ gap_ctx a b D2.
  Proof.
    intros.
    split; intros.
    - unfold gap_ctx in *; split; intros; apply H; apply in_or_app; intuition.
    - destruct H. unfold gap_ctx in *; intros.
      apply in_app_or in H1. destruct H1; eauto.
  Qed.

  Lemma gap_ctx_perm_inj : forall {a b G G'} (HG: G ≡[P] G') (HW: gap_ctx a b G), gap_ctx a b G'.
  Proof.
    intros.
    unfold gap_ctx in *.
    intros.
    pose proof (@Permutation_rel_In _ _ _ P _ _).
    symmetry in HG.
    specialize (H0 _ _ t HG).
    apply H0 in H.
    apply HW; auto.
  Qed.

  Lemma gap_ctx_perm_iff : forall {a b G G'} (HG: G ≡[P] G'), gap_ctx a b G <-> gap_ctx a b G'.
  Proof.
    intros; split; apply gap_ctx_perm_inj; auto.
    symmetry. auto.
  Qed.
  ...
  Lemma wf_typ_drop_c : forall a b c u,
      ...
      c ⊢ drop_typ a b u wf.
  Proof.
    intros a b c u HLE1 HWF. revert a b c HLE1 HWF.
    induction u; intros.
    - constructor.
    - inversion HWF; subst.
      simpl.
  ...
  Proof.
    ...
    - inversion HGC; subst; simpl; destruct LT.
      ...
      + assert (not (x < a)) by lia. apply Nat.ltb_nlt in H0. rewrite H0.
        destruct (Nat.ltb_spec x b); constructor.
        inversion HWF; subst. lia.
  ...
  Proof.
    intros a b c G HLE1 HWFG. revert a b c HLE1 HWFG.
    induction G; intros.
    - simpl. unfold wf_ctx; intros. inversion H.
  ...
  Lemma pfn_gap_drop0 : forall n a b c G D,
     ...
     gap_ctx a b G ->
     gap_ctx a b D ->
     n ⊣ c, drop_ctx a b G, drop_ctx a b D ⊢pf.
  Proof.
    intros. subst.
    replace (c + 0) with c in H1 by lia.
    repeat rewrite drop_ctx0. assumption.
  Qed.
  ...
  Proof.
    ...
    intros b c G G' D D' HG HPG HPD.
    remember (c + b) as a. revert b c G D HPG HPD Heqa.
    induction HG; intros.
    - admit.
    - admit.
    - admit.
    ...
    - admit.
    - admit.
    - eapply pfn_forall with (u := drop_typ c0 b u).
Abort.

  Lemma shift_typ_inv : forall a b t1 t2, shift_typ a b t1 = shift_typ a b t2 -> t1 = t2.
  Proof.
    intros a b t1 t2 HS. revert a b t2 HS.
    induction t1; intros; simpl in HS.
    - destruct t2; simpl in *; try discriminate; auto.
      destruct (Nat.ltb_spec x a); discriminate.
    - destruct (Nat.ltb_spec x a); destruct t2; simpl in *; try discriminate.
      + destruct (Nat.ltb_spec x0 a); injection HS; intros; subst; auto; lia.
      + destruct (Nat.ltb_spec x0 a); injection HS; intros; subst; auto; try lia.
        assert (x = x0) by lia. subst; auto.
    - destruct t2; simpl in *; try discriminate.
      + destruct (Nat.ltb_spec x a); discriminate.
      + injection HS; intros; erewrite IHt1_1, IHt1_2; eauto.
    - destruct t2; simpl in *; try discriminate.
      + destruct (Nat.ltb_spec x a); discriminate.
      + injection HS; intros; erewrite IHt1_1, IHt1_2; eauto.
    - destruct t2; simpl in *; try discriminate.
      + destruct (Nat.ltb_spec x a); discriminate.
      + injection HS; intros; erewrite IHt1; eauto.
    - destruct t2; simpl in *; try discriminate.
      + destruct (Nat.ltb_spec x a); discriminate.
      + injection HS; intros; erewrite IHt1; eauto.
    - destruct t2; simpl in *; try discriminate.
  ...
  Lemma Permutation_rel_shift_ctx_inv : forall a b D1 D2, shift_ctx a b D1 ≡[P] shift_ctx a b D2 -> D1 ≡[P] D2.
  Proof.
    intros a b D1. revert a b.
    induction D1; intros.
    - simpl in H. apply Permutation_rel_length in H. destruct D2; try discriminate. reflexivity.
    - replace (a :: D1) with ([a] ++ D1) in * by auto.
      rewrite shift_ctx_app in H.
      assert (shift_ctx a0 b D1 ++ shift_ctx a0 b [a] ≡[P] shift_ctx a0 b D2). {convertTactics.convert_multisetperm. permutation_solver. }
      eapply shift_ctx_last_exists in H0. destruct H0 as (u' & D2' & HPD1 & HPD2 & HPD3).
      symmetry in HPD2. eapply IHD1 in HPD2. 
      apply shift_typ_inv in HPD1; subst. rewrite HPD2, <- HPD3. apply Permutation_rel_exchange.
  Qed.
  
  Lemma pfn_height_weaken : forall n m c G D,
      n ⊣ c, G, D ⊢pf ->
                (n + m) ⊣ c, G, D ⊢pf.
  Proof.
    intros n m c G D HG. revert m. induction HG; intros.
    - apply pfn_id; auto.
    - replace (S n + m) with (S (n + m)) by lia.
      eapply pfn_absorb; try eassumption. eapply IHHG.
    - apply pfn_bot; auto.
    - replace (S n + m) with (S (n + m)) by lia.
      eapply pfn_one; try eassumption. eapply IHHG.
    - replace (S n + m) with (S (n + m)) by lia.
      eapply pfn_tensor; try eassumption. eapply IHHG.
      (* - replace (S (n1 `max` n2) + m) with (S ((n1 + m) `max` (n2 + m))) by lia. *)
      (* - replace (S n + m) with (S (n + m)) by lia. *)
    - replace (S (n1 + n2) + m) with (S (n1 + m + n2)) by lia.
      eapply pfn_par; try eassumption. eapply IHHG1. (* eapply IHHG2. *)
    - replace (S n + m) with (S (n + m)) by lia.
      eapply pfn_bang; try eassumption. eapply IHHG.
    - replace (S n + m) with (S (n + m)) by lia.
      eapply pfn_ques; try eassumption. eapply IHHG.
    - replace (S n + m) with (S (n + m)) by lia.
      eapply pfn_forall; try eassumption. eapply IHHG.
    - replace (S n + m) with (S (n + m)) by lia.
      eapply pfn_exists; try eassumption. eapply IHHG.
  Qed.

  Lemma lt_exists : forall n m, n < m -> exists k, n + k = m.
  Proof.
    intros n. induction n; intros.
    - exists m. lia.
    - assert (n < m) by lia.
      apply IHn in H0. destruct H0 as (k & H').
      exists (k - 1). lia.
  Qed.
  
  Lemma pfn_height_weaken_max : forall n m c G D,
      n ⊣ c, G, D ⊢pf ->
                n `max` m ⊣ c, G, D ⊢pf.
  Proof.
    intros.
    destruct (Nat.leb_spec m n).
    - replace (n `max` m) with n by lia. eassumption.
    - apply lt_exists in H0. destruct H0 as (k & H').
      replace (n `max` m) with (n + k) by lia.
      apply pfn_height_weaken; assumption.
  Qed.

  Lemma cut_admissibility_id1:
    forall m c G u u' D2 D1' D2',
    [u] ++ [dual u] ≡[P] D1' ++ [u'] ->
    D2 ≡[P] D2' ++ [dual u'] ->
    m ⊣ c, G, D2 ⊢pf ->
...
Corollary cut_admissibility_id1_plus : 
  forall n m c G u u' D2 D1' D2' a b,
    [u] ++ [dual u] ≡[P] D1' ++ [shift_typ a b u'] ->
    D2 ≡[P] D2' ++ [shift_typ a b (dual u')] ->
    m ⊣ c, G, D2 ⊢pf ->
              (n + m) ⊣ c, G, (D1' ++ D2') ⊢pf.
  Proof.
    intros.
    replace (n + m) with (m + n) by lia.
    eapply pfn_height_weaken.
    eapply cut_admissibility_id1. apply H. rewrite <- dual_shift_typ_comm. eassumption. eassumption.
  Qed.

Corollary cut_admissibility_id1_max : 
  forall n m c G u u' D2 D1' D2' a b,
    [u] ++ [dual u] ≡[P] D1' ++ [shift_typ a b u'] ->
    D2 ≡[P] D2' ++ [shift_typ a b (dual u')] ->
    m ⊣ c, G, D2 ⊢pf ->
              S (n `max` m) ⊣ c, G, (D1' ++ D2') ⊢pf.
  Proof.
    intros.
    replace (S (n `max` m)) with ((m `max` n) + 1) by lia.
    eapply pfn_height_weaken.
    eapply pfn_height_weaken_max.
    eapply cut_admissibility_id1. apply H. rewrite <- dual_shift_typ_comm. eassumption. eassumption.
  Qed.

Corollary cut_admissibility_id1' : 
  forall n m c G u u' D2 D1' D2' a b,
    [u] ++ [dual u] ≡[P] D1' ++ [shift_typ a b u'] ->
    D2 ≡[P] D2' ++ [shift_typ a b (dual u')] ->
    m ⊣ c, G, D2 ⊢pf ->
  ...
  Proof.
    intros.
    replace (S (n + m)) with (m + S n) by lia.
    eapply pfn_height_weaken.
    eapply cut_admissibility_id1. apply H. rewrite <- dual_shift_typ_comm. eassumption. eassumption.
  Qed.

Lemma cut_admissibility_id2 :
  forall m c G u u' D1 D1' D2',
    [u] ++ [dual u] ≡[P] D2' ++ [u'] ->
    D1 ≡[P] D1' ++ [dual u'] ->
    m ⊣ c, G, D1 ⊢pf ->
...
Corollary cut_admissibility_id2_plus : 
  forall n m c G u u' D1 D1' D2' a b,
    [u] ++ [dual u] ≡[P] D2' ++ [shift_typ a b u'] ->
    D1 ≡[P] D1' ++ [shift_typ a b (dual u')] ->
    m ⊣ c, G, D1 ⊢pf ->
              n + m ⊣ c, G, (D1' ++ D2') ⊢pf.
  Proof.
    intros.
    replace (n + m) with (m + n) by lia.
    eapply pfn_height_weaken.
    eapply cut_admissibility_id2. apply H. rewrite <- dual_shift_typ_comm. eassumption. eassumption.
  Qed.

Corollary cut_admissibility_id2_max : 
  forall n m c G u u' D1 D1' D2' a b,
    [u] ++ [dual u] ≡[P] D2' ++ [shift_typ a b u'] ->
    D1 ≡[P] D1' ++ [shift_typ a b (dual u')] ->
    m ⊣ c, G, D1 ⊢pf ->
...
Corollary cut_admissibility_id2' : 
  forall n m c G u u' D1 D1' D2' a b,
    [u] ++ [dual u] ≡[P] D2' ++ [shift_typ a b u'] ->
    D1 ≡[P] D1' ++ [shift_typ a b (dual u')] ->
    m ⊣ c, G, D1 ⊢pf ->
...
Lemma shift_typ_plus : forall c b1 b2 t,
    shift_typ c b1 (shift_typ c b2 t) = shift_typ c (b1 + b2) t.
Proof.
  intros. revert c b1 b2.
  induction t; intros; simpl in *; subst; auto.
  - destruct (Nat.ltb_spec x c); simpl.
    + apply Nat.ltb_lt in H. rewrite H; auto.
    + assert (not (b2 + x < c)) by lia. apply Nat.ltb_nlt in H0. rewrite H0. replace (b1 + (b2 + x)) with (b1 + b2 + x) by lia. auto.
  - rewrite IHt1, IHt2; auto.
  - rewrite IHt1, IHt2; auto.
  - rewrite IHt; auto.
  - rewrite IHt; auto.
...
Lemma shift_ctx_plus : forall c b1 b2 G,
    shift_ctx c b1 (shift_ctx c b2 G) = shift_ctx c (b1 + b2) G.
  intros. revert c b1 b2.
  induction G; intros; simpl in *; subst; auto.
  rewrite shift_typ_plus. rewrite IHG. auto.
Qed.
  ...
  (*                   y = n0 + m0 *)
  (*                   → ∀ b : nat, *)
  (*                       cut_admissibility_norm n0 m0 u b *)
  (*                       ∧ ((∀ n'0 m' b0 : nat, cut_admissibility_norm n'0 m' u b0) → cut_admissibility_bang n0 m0 u b) *)
  (*                         ∧ ((∀ n'0 m' b0 : nat, cut_admissibility_norm n'0 m' u b0) → cut_admissibility_ques n0 m0 u b)) *)
  (*            → n' = n + m *)
  (*              → ∀ b : nat, *)
  (*                  cut_admissibility_norm n m u1 b *)
  (*                  ∧ ((∀ n'0 m' b0 : nat, cut_admissibility_norm n'0 m' u1 b0) → cut_admissibility_bang n m u1 b) *)
  (*                    ∧ ((∀ n'0 m' b0 : nat, cut_admissibility_norm n'0 m' u1 b0) → cut_admissibility_ques n m u1 b) *)
  (* IHu2 : ∀ n' n m : nat, *)
  (*          (∀ y : nat, y < n' → Acc lt y) *)
  (*          → (∀ y : nat, *)
  (*               y < n' *)
  ...
  (*            → n' = n + m *)
  (*              → ∀ b : nat, *)
  (*                  cut_admissibility_norm n m u2 b *)
  (*                  ∧ ((∀ n'0 m' b0 : nat, cut_admissibility_norm n'0 m' u2 b0) → cut_admissibility_bang n m u2 b) *)
  (*                    ∧ ((∀ n'0 m' b0 : nat, cut_admissibility_norm n'0 m' u2 b0) → cut_admissibility_ques n m u2 b) *)
  (* IHY : ∀ y : nat, y < n' → Acc lt y *)
  (* IHWF : ∀ y : nat, *)
  (*          y < n' *)
  (*          → ∀ (n m : nat) (u : typ), *)
  (*              y = n + m *)
  (*              → ∀ b : nat, *)
  (*                  cut_admissibility_norm n m u b *)
  (*                  ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u b0) → cut_admissibility_bang n m u b) *)
  (*                    ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u b0) → cut_admissibility_ques n m u b) *)
  (* Heqo : n' = n + m *)
  (* ============================ *)
  (* cut_admissibility_norm n m (u1 ⊗ u2) b *)

  
  Lemma cut_admissibility_tensor_norm :
    forall u1 u2 n' n m b
      (Heqn' : n' = n + m)
      (IHY : ∀ y : nat, y < n' → Acc lt y)
      (IHWF : ∀ y : nat,
          y < n'
...
Proof.
    ...
    destruct HG1; subst.
    - eapply cut_admissibility_id1_plus; eassumption.
    - assert (HM1: n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3); unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G' (D ++ [t]) (D1' ++ [t]) D2 D2'). listSimpl.
      assert (D ++ [t] ≡[P] (D1' ++ [t]) ++ shift_ctx 0 b [u1 ⊗ u2]). {rewrite HPD1. clear_permrel. apply Permutation_rel_assoc_swap. }
      specialize (IHWF1 H1 HPD2 HPG1 HPG2 HG1 HG2).
      replace (S n + m) with (S (n + m)) by lia.
      eapply pfn_absorb; try eassumption.
      eapply pfn_perm_rel. reflexivity. apply Permutation_rel_assoc_swap.
      assumption.
    ...
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H.
      ...
      assert (D' ≡[P] E1 ++ shift_ctx 0 b[u1 ⊗ u2]). {rewrite HPE2, <- HPE3. reflexivity. }
    ...
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H.
      ...
      + injection HPE1. intros. subst. clear HPE1.
        ...
        *
          ...
          replace (S n + n0) with (n0 + S n) by lia.
          eapply cut_admissibility_id2_plus. eassumption. rewrite dual_involutive. eassumption.
         eapply pfn_perm_rel. reflexivity. rewrite HPD1, HPE2. reflexivity. apply HG1.
        * assert (HM1: (S n) + n0 < S n + S n0) by lia.
           specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3); unfold_admissibilityH IHWF1.
           specialize (IHWF1 c G' D D1' (D0 ++ [t]) (D2' ++ [t])); listSimpl.
           assert (D0 ++ [t] ≡[P] (D2' ++ [t]) ++ shift_ctx 0 b [dual (u1 ⊗ u2)]). {rewrite HPD2. apply Permutation_rel_assoc_swap. }
           replace (S n + S n0) with (S (S n + n0)) by lia.
           eapply pfn_tensor in HG1. 2: { rewrite <- HPE2, <- HPD1. reflexivity. }
           specialize (IHWF1 HPD1 H1 HPG1 HPG2 HG1 HG2).
           eapply pfn_absorb. 3: {rewrite <- app_assoc. replace (S n + S n0) with (S (S n + n0)) by lia. eapply IHWF1. } 2: {eapply H0. }. wf_ctx_solver.
        ...
        * rewrite HPD2 in H. apply Permutation_rel_split_last in H.
           ...
           specialize (IHWF1 HPD1 HPF2 HPG1 HPG2 HG1 HG2).
           replace (S n + S n0) with (S (S n + n0)) by lia.
           eapply pfn_one. replace (S n + S n0) with (S (S n + n0)) by lia.
           apply IHWF1. rewrite HPF1, HPF3, <- app_assoc. reflexivity.
        * rewrite HPD2 in H. apply Permutation_rel_split_last in H.
          destruct H as [[HPF1 HPF2] | [F1 [F2 [HPF1 [HPF2 HPF3]]]]]; try discriminate.
          assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3); unfold_admissibilityH IHWF1.
          specialize (IHWF1 c G D D1' (D'0 ++ [t] ++ [u]) (F2 ++ [t] ++ [u])); listSimpl.
          assert (HPD'1: D'0 ++ [t] ++ [u] ≡[P] (F2 ++ [t] ++ [u]) ++ shift_ctx 0 b[dual (u1 ⊗ u2)]).
          {rewrite HPF2. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. }
          eapply pfn_tensor in HG1.
          2: {rewrite <- HPE2. eassumption. }
          specialize (IHWF1 HPD1 HPD'1 HPG1 HPG1 HG1 HG2).
          replace (S n + S n0) with (S (S n + n0)) by lia.
          eapply pfn_tensor. replace (S n + S n0) with (S (S n + n0)) by lia. rewrite app_assoc in IHWF1. eassumption. rewrite HPF1, HPF3, app_assoc. reflexivity.
        * rewrite HPD2 in H. rewrite app_assoc in H. apply Permutation_rel_split_last in H.
          ...
          --
            ...
            assert (IHY1 : ∀ y : nat, y < (S n + n1) → Acc lt y). {intros. apply IHY; lia. }
            destruct (IHu1 (n + n1) n n1) with (b := b) as (IHu1_1 & _ & _); auto. intros; apply IHY; lia. intros; eapply IHWF; try eassumption; lia. unfold_admissibilityH IHu1_1.
            (* specialize (IHu1 n n1 b) as (IHu1_1 & _ & _); unfold_admissibilityH IHu1_1. *)
            specialize (IHu1_1 c G (D' ++ [shift_typ 0 b u1] ++ [shift_typ 0 b u2]) (D' ++ [shift_typ 0 b u2]) (D1 ++ [shift_typ 0 b (dual u1)]) D1); listSimpl.
            assert (HPD'1: D' ++ [shift_typ 0 b u1] ++ [shift_typ 0 b u2] ≡[P] (D' ++ [shift_typ 0 b u2]) ++ [shift_typ 0 b u1]). {rewrite app_assoc. apply Permutation_rel_assoc_swap. }
            assert (HPD'2: D1 ++ [shift_typ 0 b (dual u1)] ≡[P] D1 ++ [shift_typ 0 b (dual u1)]) by reflexivity.
            (* specialize (IHu1_1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2_1) as (k1 & IHu1_1). *)
            specialize (IHu1_1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2_1).
            clear HPD'1 HPD'2.
            destruct (IHu2 ((n + n1) + n2) (n + n1) n2) with (b := b) as (IHu2_1 & _ & _); auto. intros; apply IHY; lia. intros; eapply IHWF; try eassumption; lia. unfold_admissibilityH IHu2_1.
            (* specialize (IHu2 k1 n2 b) as (IHu2_1 & _ & _); unfold_admissibilityH IHu2_1. *)
            specialize (IHu2_1 c G ((D' ++ [shift_typ 0 b u2]) ++ D1) (D' ++ D1) (D2 ++ [shift_typ 0 b (dual u2)]) D2); listSimpl.
            assert (HPD'1: (D' ++ [shift_typ 0 b u2]) ++ D1 ≡[P] (D' ++ D1) ++ [shift_typ 0 b u2]) by apply Permutation_rel_assoc_swap.
            assert (HPD'2 : D2 ++ [shift_typ 0 b (dual u2)] ≡[P] D2 ++ [shift_typ 0 b (dual u2)]) by reflexivity.
            specialize (IHu2_1 HPD'1 HPD'2 HPG1 HPG2 IHu1_1 HG2_2).
          (* specialize (IHu2_1 HPD'1 HPD'2 HPG1 HPG2 IHu1_1 HG2_2) as (k2 & IHu2_1). *)
            replace (S n + S (n1 + n2)) with ((n + n1 + n2) + 2) by lia. eapply pfn_height_weaken.
            eapply pfn_perm_rel_iff. reflexivity. rewrite HPE2, HPF2, app_assoc. reflexivity. assumption.
         ...
         -- 
            ...
            ++
              assert (HM1 : S n + n1 < S n + S (n1 + n2)) by lia.
              (* assert (HM1: S n + n1 < S n + S (Nat.max n1 n2)) by lia. *)
              specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
              specialize (IHWF1 c G D D1' (D1 ++ [t]) (I1 ++ [t])); listSimpl.
              assert (HPD'1: D1 ++ [t] ≡[P] (I1 ++ [t]) ++ [shift_typ 0 b (dual (u1 ⊗ u2))]). {rewrite HPI1. apply Permutation_rel_assoc_swap. }
              eapply pfn_tensor in HG1. 2: {rewrite <- HPE2. eassumption. }
              specialize (IHWF1 HPD1 HPD'1 HPG1 HPG2 HG1 HG2_1).
              (* specialize (IHWF1 HPD1 HPD'1 HPG1 HPG2 HG1 HG2_1) as (k & IHWF1). *)
              eapply pfn_perm_rel_iff. reflexivity.
              assert (D1' ++ D2' ≡[P] (D1' ++ I1) ++ D2 ++ [t_par t u]). {rewrite HPF1, HPF3, <- HPI2. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. }
              apply H.
              replace (S n + S (n1 + n2)) with (S ((S n + n1) + n2)) by lia.
              eapply pfn_par.
              ** rewrite app_assoc in IHWF1. eapply IHWF1.
              ** eapply HG2_2.
              ** reflexivity.
            ...
            ++
              assert (HM1: S n + n2 < S n + S (n1 + n2)) by lia.
              specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
              specialize (IHWF1 c G D D1' (D2 ++ [u]) (I2 ++ [u])); listSimpl.
              assert (HPD'1: D2 ++ [u] ≡[P] (I2 ++ [u]) ++ [shift_typ 0 b (dual (u1 ⊗ u2))]). {rewrite HPI1. apply Permutation_rel_assoc_swap. }
              eapply pfn_tensor in HG1. 2: {rewrite <- HPE2. eassumption. }
              specialize (IHWF1 HPD1 HPD'1 HPG1 HPG2 HG1 HG2_2).
              eapply pfn_perm_rel_iff. reflexivity.
              (* specialize (IHWF1 HPD1 HPD'1 HPG1 HPG2 HG1 HG2_2) as (k & IHWF1). *)
              (* eapply pfn_perm_rel_iff_exists. reflexivity. *)
              assert (D1' ++ D2' ≡[P] D1 ++ (D1' ++ I2) ++ [t_par t u]). {rewrite HPF1, HPF3, <- HPI2. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. }
              apply H.
              replace (S n + S (n1 + n2)) with (S (n1 + (S (n + n2)))) by lia.
              eapply pfn_par.
              ** apply HG2_1.
        ...
        * rewrite HPD2 in H. apply Permutation_rel_split_last in H.
           ...
           specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3); unfold_admissibilityH IHWF1.
           specialize (IHWF1 c (G ++ [t]) D D1' D1 F1); listSimpl.
           assert (HPD'1 : D1 ≡[P] F1 ++ [shift_typ 0 b (dual (u1 ⊗ u2))]). {rewrite HPF2, <- HPF3. reflexivity. }
           eapply pfn_tensor in HG1. 2: {rewrite <- HPE2. apply HPD1. }
           assert (HG'1: S n ⊣ c, (G ++ [t]), D ⊢pf). {eapply pfn_weakening. reflexivity. pfn_wf_ctx_solver. assumption. }
           assert (HPG'1 : G ++ [t] ≡[P] G ++ [t]) by reflexivity.
           specialize (IHWF1 HPD1 HPD'1 HPG'1 HPG'1 HG'1 HG2).
           replace (S n + S n0) with (S (S n + n0)) by lia.
           eapply pfn_bang. 2: {rewrite HPF1, app_assoc. reflexivity. } assumption.
        ...
        * 
          ...
          assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3); unfold_admissibilityH IHWF1.
          specialize (IHWF1 c G D D1' (D1 ++ [typ_subst 0 u t]) (F1 ++ [typ_subst 0 u t])); listSimpl.
          assert (HPD'1: D1 ++ [typ_subst 0 u t] ≡[P] (F1 ++ [typ_subst 0 u t]) ++ [shift_typ 0 b (dual (u1 ⊗ u2))]). {rewrite HPF2, <- HPF3. apply Permutation_rel_assoc_swap. }
          replace (S n + S n0) with (S (S n + n0)) by lia.
          eapply pfn_tensor in HG1. 2: {rewrite <- HPE2. eassumption. }
          specialize (IHWF1 HPD1 HPD'1 HPG1 HPG2 HG1 HG2).
          eapply pfn_forall; try eassumption. rewrite app_assoc in IHWF1. eassumption. rewrite HPF1, HPF3, app_assoc. reflexivity.
        ...
        * rewrite HPD2 in H. apply Permutation_rel_split_last in H.
          ...
          assert (HM1: S n + n0 < S n + S n0) by lia.
          eapply pfn_tensor in HG1. 2: {rewrite <- HPE2. eassumption. }
          specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl (b + 1)) as (IHWF1 & IWHF2 & IHWFF3); unfold_admissibilityH IHWF1.
          apply (pfn_shift_inv 0 1) in HG1; try lia.
          specialize (IHWF1 (1 + c) (shift_ctx 0 1 G) (shift_ctx 0 1 D) (shift_ctx 0 1 D1') (shift_ctx 0 1 D1 ++ [u]) (shift_ctx 0 1 F2 ++ [u])). listSimpl.
          assert (HS1 : shift_ctx 0 1 D ≡[P] shift_ctx 0 1 D1' ++ shift_ctx 0 (b + 1) [u1 ⊗ u2]).
          {rewrite HPD1, shift_ctx_app. simpl. repeat rewrite shift_typ_plus. replace (1 + b) with (b + 1) by lia. reflexivity. }
          assert (HS2 : shift_ctx 0 1 D1 ++ [u] ≡[P] (shift_ctx 0 1 F2 ++ [u]) ++ shift_ctx 0 (b + 1) [dual (u1 ⊗ u2)]).
          {rewrite HPF2, shift_ctx_app, Permutation_rel_assoc_swap. simpl. repeat rewrite shift_typ_plus. replace (1 + b) with (b + 1) by lia. reflexivity. }
          assert (HSG : shift_ctx 0 1 G ≡[P] shift_ctx 0 1 G) by reflexivity.
          replace (c + 1) with (1 + c) in * by lia.
          specialize (IHWF1 HS1 HS2 HSG HSG HG1 HG2).
          replace (S n + S n0) with (S (S n + n0)) by lia.
          eapply pfn_exists.
          -- rewrite app_assoc, <- shift_ctx_app in IHWF1. eapply IHWF1.
          -- rewrite HPF1, HPF3, app_assoc. reflexivity.
      ...
      + subst.
        ...
        assert (HPD'1: (D' ++ [t] ++ [u]) ≡[P] (E1 ++ [t] ++ [u]) ++ [shift_typ 0 b (u1 ⊗ u2)]). {rewrite HPE2, <- HPE3. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. }
        specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2).
        eapply pfn_tensor. 2: {assert (D1' ++ D2' ≡[P] (E1 ++ D2') ++ [t ⊗ u]). {rewrite HPE1. apply Permutation_rel_assoc_swap. } eassumption. }
    ...
    - rewrite HPD1, app_assoc in H. apply Permutation_rel_split_last in H.
      ...
      + assert (HM1: n1 + m < S (n1 + n2) + m) by lia.
        specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). unfold cut_admissibility_norm, cut_admissibility_t in IHWF1.
        specialize (IHWF1 c G (D1 ++ [t]) (F1 ++ [t]) D2 D2'); listSimpl.
        assert (HPD'1: D1 ++ [t] ≡[P] (F1 ++ [t]) ++ [shift_typ 0 b (u1 ⊗ u2)]). {rewrite HPF1. apply Permutation_rel_assoc_swap. }
        specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1_1 HG2).
        assert (IHWF'1: n1 + m ⊣ c, G, (F1 ++ D2') ++ [t] ⊢pf). {eapply pfn_perm_rel. reflexivity. apply Permutation_rel_assoc_swap. assumption. }
        eapply pfn_perm_rel_iff. reflexivity. assert (D1' ++ D2' ≡[P] (F1 ++ D2') ++ D0 ++ [t_par t u]). {rewrite HPE1, HPE3, <- HPF2. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. } eassumption.
        replace (S (n1 + n2) + m) with (S ((n1 + m) + n2)) by lia.
        eapply pfn_par.
        * eapply IHWF'1.
        * eapply HG1_2.
        * reflexivity.
      + assert (HM1: n2 + m < S (n1 + n2) + m) by lia.
        specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3); unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G (D0 ++ [u]) (F2 ++ [u]) D2 D2'); listSimpl.
        assert (HPD'1: D0 ++ [u] ≡[P] (F2 ++ [u]) ++ [shift_typ 0 b (u1 ⊗ u2)]). {rewrite HPF1. apply Permutation_rel_assoc_swap. }
        specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1_2 HG2).
        assert (IHWF'1: n2 + m ⊣ c, G, (F2 ++ D2') ++ [u] ⊢pf). {eapply pfn_perm_rel. reflexivity. apply Permutation_rel_assoc_swap. assumption. }
        eapply pfn_perm_rel_iff. reflexivity. assert (D1' ++ D2' ≡[P] D1 ++ (F2 ++ D2') ++ [t_par t u]). {rewrite HPE1, HPE3, <- HPF2. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. } eassumption.
        replace (S (n1 + n2) + m) with (S (n1 + (n2 + m))) by lia.
        eapply pfn_par.
        * eapply HG1_1.
        * eapply IHWF'1.
        * reflexivity.
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H.
      ...
      assert (HPG'1: G ++ [t] ≡[P] G ++ [t]) by reflexivity.
      specialize (IHWF1 HPE2 HPD2 HPG'1 HPG'1 HG1 HG'1).
      eapply pfn_perm_rel. reflexivity. rewrite HPE1. apply Permutation_rel_assoc_swap.
      replace (S n + m) with (S (n + m)) by lia.
    ...
    - rewrite HPD1 in H0. apply Permutation_rel_split_last in H0.
      ...
      subst. assert (HM1: n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3); unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G (D1 ++ [typ_subst 0 u t]) (E1 ++ [typ_subst 0 u t]) D2 D2'); listSimpl.
      assert (HPD'1: D1 ++ [typ_subst 0 u t] ≡[P] (E1 ++ [typ_subst 0 u t]) ++ [shift_typ 0 b (u1 ⊗ u2)]). {rewrite HPE2, <- HPE3. apply Permutation_rel_assoc_swap. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2).
      eapply pfn_perm_rel. reflexivity. rewrite HPE1. apply Permutation_rel_assoc_swap.
      replace (S n + m) with (S (n + m)) by lia.
    ...
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H.
      destruct H as [[HPE1 HPE2] | [E1 [E2 [HPE1 [HPE2 HPE3]]]]]; try discriminate.
      subst. assert (HM1: n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl (b + 1)) as (IHWF1 & IHWF2 & IHWF3); unfold_admissibilityH IHWF1.
      specialize (IHWF1 (1 + c) (shift_ctx 0 1 G) (shift_ctx 0 1 D1 ++ [u]) (shift_ctx 0 1 E1 ++ [u]) (shift_ctx 0 1 D2) (shift_ctx 0 1 D2')); listSimpl.
      eapply (pfn_shift_inv 0 1) in HG2; try lia.
      assert (HS1 : shift_ctx 0 1 D1 ++ [u] ≡[P] (shift_ctx 0 1 E1 ++ [u]) ++ shift_ctx 0 (b + 1) [u1 ⊗ u2]).
      {rewrite HPE2, HPE3, shift_ctx_app, Permutation_rel_assoc_swap. simpl. repeat rewrite shift_typ_plus. replace (1 + b) with (b + 1) by lia. reflexivity. }
      assert (HS2 : shift_ctx 0 1 D2 ≡[P] shift_ctx 0 1 D2' ++ shift_ctx 0 (b + 1) [dual (u1 ⊗ u2)]).
      {rewrite HPD2, shift_ctx_app. simpl. repeat rewrite shift_typ_plus. replace (1 + b) with (b + 1) by lia. reflexivity. }
      assert (HSG : shift_ctx 0 1 G ≡[P] shift_ctx 0 1 G) by reflexivity.
      replace (c + 1) with (1 + c) in * by lia.
      specialize (IHWF1 HS1 HS2 HSG HSG HG1 HG2).
      replace (S n + m) with (S (n + m)) by lia.
      eapply pfn_exists.
      2: {rewrite HPE1. eapply Permutation_rel_assoc_swap. }
      eapply pfn_perm_rel. reflexivity. rewrite shift_ctx_app. eapply Permutation_rel_assoc_swap. assumption.
Qed.
...
Lemma cut_admissibility_tensor_bang :
  forall u1 u2 n' n m b
    ...
    (IHu2 : ∀ n' n m : nat,
        ...
        → ∀ b : nat,
            ...
            ∧ ((∀ n'0 m' b0 : nat, cut_admissibility_norm n'0 m' u2 b0) → cut_admissibility_bang n m u2 b)
            ∧ ((∀ n'0 m' b0 : nat, cut_admissibility_norm n'0 m' u2 b0) → cut_admissibility_ques n m u2 b))
    (HC: forall n m b0, cut_admissibility_norm n m (u1 ⊗ u2) b0), cut_admissibility_bang n m (u1 ⊗ u2) b.
  Proof.
    intros. unfold cut_admissibility_bang, cut_admissibility_t.
    intros c G1 G2 D1 D2 HPD1 HPD2 HPG1 HPG2 HG1 HG2. listSimpl. 
    destruct HG2; subst.
    - exists 0. apply pfn_id; pfn_wf_ctx_solver.
    - rewrite HPG2 in H0. apply Permutation_rel_split_last in H0.
      ...
      + 
        ...
        specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). specialize (IHWF2 HC). unfold_admissibilityH IHWF2.
        specialize (IHWF2 c G1 G' D1 (D ++ [shift_typ 0 b (dual (u1 ⊗ u2))])); listSimpl.
        assert (HPD'1: D ++ [shift_typ 0 b (dual (u1 ⊗ u2))] ≡[P] D ++ [shift_typ 0 b (dual (u1 ⊗ u2))]) by reflexivity.
        specialize (IHWF2 HPD1 HPD'1 HPG1 HPG2 HG1 HG2). destruct IHWF2 as (k & IHWF2).
        unfold_admissibilityH HC. specialize (HC n k b c G1 D1 [] (D ++ [shift_typ 0 b (dual (u1 ⊗ u2))]) D); listSimpl.
        specialize (HC HPD1 HPD'1 HPG1 HPG1 HG1 IHWF2).
        eexists; eauto.
      + assert (HM1 : n + n0 < n + S n0) by lia.
        specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF2 HC); unfold_admissibilityH IHWF2.
    ...
    - pose proof IHWF as IHWF'.
      assert (HM1: n + n1 < n + S (n1 + n2)) by lia.
      assert (HM2: n + n2 < n + S (n1 + n2)) by lia.
      specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). specialize (IHWF2 HC); unfold_admissibilityH IHWF2.
      ...
      specialize (IHWF' _ HM2 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF'1 & IHWF'2 & IHWF'3). specialize (IHWF'2 HC); unfold_admissibilityH IHWF'2.
      ...
      exists (S (k1 + k2)). eapply pfn_par; try eassumption.
    - assert (HM1: n + n0 < n + S n0) by lia.
      specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF2 HC); unfold_admissibilityH IHWF2.
      specialize (IHWF2 c (G1 ++ [t]) (G ++ [t]) D1 D0); listSimpl.
      assert (HPD'1 : D0 ≡[P] D0) by reflexivity.
      assert (HPG'1 : G1 ++ [t] ≡[P] G1 ++ [t]) by reflexivity.
      assert (HPG'2 : G ++ [t] ≡[P] (G1 ++ [t]) ++ [shift_typ 0 b (dual (u1 ⊗ u2))]) by (rewrite HPG2; apply Permutation_rel_assoc_swap).
    ...
    - assert (HM1: n + n0 < n + S n0) by lia.
      specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF2 HC); unfold_admissibilityH IHWF2.
      specialize (IHWF2 c G1 G D1 (D0 ++ [typ_subst 0 u t])); listSimpl.
      assert (HPD'1: D0 ++ [typ_subst 0 u t] ≡[P] D0 ++[typ_subst 0 u t]) by reflexivity.
      specialize (IHWF2 HPD1 HPD'1 HPG1 HPG2 HG1 HG2) as (k & IHWF2).
      exists (S k). eapply pfn_forall. eassumption. apply IHWF2. assumption.
    - assert (HM1 : n + n0 < n + S n0) by lia.
      specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl (b + 1)) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF2 HC); unfold_admissibilityH IHWF2.
      eapply (pfn_shift_inv 0 1) in HG1; try lia.
      specialize (IHWF2 (1 + c) (shift_ctx 0 1 G1) (shift_ctx 0 1 G) (shift_ctx 0 1 D1) (shift_ctx 0 1 D0 ++ [u])); listSimpl.
      assert (HS1 : shift_ctx 0 1 D1 ≡[P] shift_ctx 0 (b + 1) [u1 ⊗ u2]).
      {rewrite HPD1. simpl; repeat rewrite shift_typ_plus. replace (1 + b) with (b + 1) by lia. reflexivity. }
      assert (HS2 : shift_ctx 0 1 D0 ++ [u] ≡[P] shift_ctx 0 1 D0 ++ [u]) by reflexivity.
      assert (HSG1: shift_ctx 0 1 G1 ≡[P] shift_ctx 0 1 G1) by reflexivity.
      assert (HSG2: shift_ctx 0 1 G ≡[P] shift_ctx 0 1 G1 ++ shift_ctx 0 (b + 1) [dual (u1 ⊗ u2)]).
      {rewrite HPG2, shift_ctx_app. simpl. repeat rewrite shift_typ_plus. replace (1 + b) with (b + 1) by lia. reflexivity. }
      replace (c + 1) with (1 + c) in * by lia.
      specialize (IHWF2 HS1 HS2 HSG1 HSG2 HG1 HG2). destruct IHWF2 as (k & IHWF2).
      exists (S k). eapply pfn_exists; eassumption.
  Qed.
    ...
    forall u1 u2 n' n m b
      ...
      (IHu2 : ∀ n' n m : nat,
          ...
          → ∀ b : nat,
              ...
              ∧ ((∀ n'0 m' b0 : nat, cut_admissibility_norm n'0 m' u2 b0) → cut_admissibility_bang n m u2 b)
              ∧ ((∀ n'0 m' b0 : nat, cut_admissibility_norm n'0 m' u2 b0) → cut_admissibility_ques n m u2 b))
      (HC: forall n m b0, cut_admissibility_norm n m (u1 ⊗ u2) b0), cut_admissibility_ques n m (u1 ⊗ u2) b.
  Proof.
    ...
    - rewrite HPG1 in H0. apply Permutation_rel_split_last in H0.
      ...
      + 
        subst.
        assert (HM1: n + m < S n + m) by lia.
        specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). specialize (IHWF3 HC). unfold_admissibilityH IHWF3.
        specialize (IHWF3 c G' G2 (D ++ [shift_typ 0 b (u1 ⊗ u2)]) D2); listSimpl.
        assert (HPD'1: D ++ [shift_typ 0 b (u1 ⊗ u2)] ≡[P] D ++ [shift_typ 0 b (u1 ⊗ u2)]) by reflexivity.
        specialize (IHWF3 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF3).
        unfold_admissibilityH HC. specialize (HC k m b c G2 (D ++ [shift_typ 0 b(u1 ⊗ u2)]) D D2 []); listSimpl.
        specialize (HC HPD'1 HPD2 HPG2 HPG2 IHWF3 HG2).
        eexists; eauto.
      + assert (HM1 : n + m < S n + m) by lia.
        specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF3 HC); unfold_admissibilityH IHWF3.
    ...
    - pose proof IHWF as IHWF'.
      assert (HM1: n1 + m < S (n1 + n2) + m) by lia.
      assert (HM2: n2 + m < S (n1 + n2) + m) by lia.
      specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). specialize (IHWF3 HC); unfold_admissibilityH IHWF3.
      ...
      specialize (IHWF' _ HM2 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF'1 & IHWF'2 & IHWF'3). specialize (IHWF'3 HC); unfold_admissibilityH IHWF'3.
    ...
    - assert (HM1: n + m < S n + m) by lia.
      ...
      assert (HPG'2 : G ++ [t] ≡[P] (G2 ++ [t]) ++ [shift_typ 0 b (u1 ⊗ u2)]) by (rewrite HPG1; apply Permutation_rel_assoc_swap).
    ...
    - assert (HM1: n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl b) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF3 HC); unfold_admissibilityH IHWF3.
      specialize (IHWF3 c G G2 (D1 ++ [typ_subst 0 u t]) D2); listSimpl.
      assert (HPD'1: D1 ++ [typ_subst 0 u t] ≡[P] D1 ++[typ_subst 0 u t]) by reflexivity.
      specialize (IHWF3 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF3).
      exists (S k). eapply pfn_forall. eassumption. apply IHWF3. assumption.
    - assert (HM1 : n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ (u1 ⊗ u2) eq_refl (1 + b)) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF3 HC); unfold_admissibilityH IHWF3.
      specialize (IHWF3 (1 + c) (shift_ctx 0 1 G) (shift_ctx 0 1 G2) (shift_ctx 0 1 D1 ++ [u]) (shift_ctx 0 1 D2)); listSimpl.
      assert (HSD1: shift_ctx 0 1 D1 ++ [u] ≡[P] shift_ctx 0 1 D1 ++ [u]) by reflexivity.
      assert (HSD2: shift_ctx 0 1 D2 ≡[P] shift_ctx 0 (1 + b) [dual (u1 ⊗ u2)]).
      {rewrite HPD2. simpl; repeat rewrite shift_typ_plus. reflexivity. }
      assert (HSG1 : shift_ctx 0 1 G ≡[P] shift_ctx 0 1 G2 ++ shift_ctx 0 (1 + b) [u1 ⊗ u2]).
      {rewrite HPG1, shift_ctx_app. simpl. repeat rewrite shift_typ_plus. reflexivity. }
      assert (HSG2 : shift_ctx 0 1 G2 ≡[P] shift_ctx 0 1 G2) by reflexivity.
      eapply (pfn_shift_inv 0 1) in HG2; try lia.
      replace (c + 1) with (1 + c) in * by lia.
      specialize (IHWF3 HSD1 HSD2 HSG1 HSG2 HG1 HG2). destruct IHWF3 as (k & IHWF3).
      exists (S k). eapply pfn_exists; eassumption.
  Qed.
  ...
  Lemma cut_admissibility_tensor :
    forall u1 u2 n' n m b
      ...
      (Heqo : n' = n + m),
      cut_admissibility_norm n m (u1 ⊗ u2) b
      ∧ ((∀ n'0 m' b0 : nat, cut_admissibility_norm n'0 m' (u1 ⊗ u2) b0) → cut_admissibility_bang n m (u1 ⊗ u2) b)
      ∧ ((∀ n'0 m' b0 : nat, cut_admissibility_norm n'0 m' (u1 ⊗ u2) b0) → cut_admissibility_ques n m (u1 ⊗ u2) b).
  Proof.
    intros. repeat split.
    - eapply cut_admissibility_tensor_norm; eassumption.
    - eapply cut_admissibility_tensor_bang; eassumption.
    - eapply cut_admissibility_tensor_ques; eassumption.
  Qed.
  ...
  Lemma cut_admissibility_bang_norm:
    forall u n' n m b
      ...
      (IHu : ∀ n' n m : nat,
          ...
          → ∀ b : nat,
              cut_admissibility_norm n m u b
              ∧ ((∀ n'0 m' b0 : nat, cut_admissibility_norm n'0 m' u b0) → cut_admissibility_bang n m u b)
              ∧ ((∀ n'0 m' b0 : nat, cut_admissibility_norm n'0 m' u b0) → cut_admissibility_ques n m u b)),
      cut_admissibility_norm n m ([!] u) b.
  Proof.
    intros. unfold cut_admissibility_norm, cut_admissibility_t.
    intros c G D1 D1' D2 D2' HPD1 HPD2 HPG1 HPG2 HG1 HG2.
    ...
    - eapply cut_admissibility_id1_plus; eassumption.
    - subst. assert (HM1: n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ ([!]u) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G' (D ++ [t]) (D1' ++ [t]) D2 D2').
      assert (HPD'1: D ++ [t] ≡[P] (D1' ++ [t]) ++ [shift_typ 0 b ([!] u)]). {rewrite HPD1. apply Permutation_rel_assoc_swap. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2).
      replace (S n + m) with (S (n + m)) by lia.
      eapply pfn_absorb. eassumption. eassumption.
      eapply pfn_perm_rel. reflexivity. eapply Permutation_rel_assoc_swap. assumption.
    - symmetry in HPD1. apply Permutation_rel_singleton_nil in HPD1 as (_ & Hcontra). discriminate.
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H.
      destruct H as [[HPE1 HPE2] | [E1 [E2 [HPE1 [HPE2 HPE3]]]]]; try discriminate.
      ...
      specialize (IHWF1 c G D' E1 D2 D2').
      rewrite <- HPE3 in HPE2.
      specialize (IHWF1 HPE2 HPD2 HPG1 HPG2 HG1 HG2).
      replace (S n + m) with (S (n + m)) by lia.
      eapply pfn_one. 2: {rewrite HPE1. eapply Permutation_rel_assoc_swap. } assumption.
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H.
      destruct H as [[HPE1 HPE2] | [E1 [E2 [HPE1 [HPE2 HPE3]]]]]; try discriminate.
      subst. assert (HM1: n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ ([!]u) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G (D' ++ [t] ++ [u0]) (E1 ++ [t] ++ [u0]) D2 D2').
      assert (HPD'1: D' ++ [t] ++ [u0] ≡[P] (E1 ++ [t] ++ [u0] ) ++ [shift_typ 0 b ([!] u)]). {rewrite HPE2, <- HPE3. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2).
      replace (S n + m) with (S (n + m)) by lia.
      eapply pfn_tensor. 2: {rewrite HPE1. apply Permutation_rel_assoc_swap. }
    ...
    - rewrite HPD1, app_assoc in H. apply Permutation_rel_split_last in H.
      ...
      + subst. assert (HM1: n1 + m < S (n1 + n2) + m) by lia.
        specialize (IHWF _ HM1 _ _ ([!] u) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G (D1 ++ [t]) (F1 ++ [t]) D2 D2').
        assert (HPD'1: D1 ++ [t] ≡[P] (F1 ++ [t]) ++ [shift_typ 0 b ([!] u)]). {rewrite HPF1. apply Permutation_rel_assoc_swap. }
        specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1_1 HG2).
        replace (S (n1 + n2) + m) with (S ((n1 + m) + n2)) by lia.
        eapply pfn_par.
        * 
      ...
      + subst. assert (HM1: n2 + m < S (n1 + n2) + m) by lia.
        specialize (IHWF _ HM1 _ _ ([!]u) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G (D0 ++ [u0]) (F2 ++ [u0]) D2 D2').
        assert (HPD'1: D0 ++ [u0] ≡[P] (F2 ++ [u0]) ++ [shift_typ 0 b ([!] u)]). {rewrite HPF1. apply Permutation_rel_assoc_swap. }
        specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1_2 HG2).
        replace (S (n1 + n2) + m) with (S (n1 + (n2 + m))) by lia.
        eapply pfn_par.
        * 
    ...
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H.
      ...
      + injection HPE1. intros; subst. clear HPE1.
        ...
        * assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ ([!] u) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 c G' (D1 ++ [shift_typ 0 b ([!]u)]) D1' (D0 ++ [t]) (D2' ++ [t])).
          assert (HPD'1: D1 ++ [shift_typ 0 b ([!]u)] ≡[P] D1' ++ [shift_typ 0 b ([!] u)]). {rewrite HPE2. reflexivity. }
          assert (HPD'2: D0 ++ [t] ≡[P] (D2' ++ [t]) ++ [shift_typ 0 b (dual ([!] u))]). {rewrite HPD2. apply Permutation_rel_assoc_swap. }
          eapply pfn_bang in HG1. 2: {reflexivity. }
          specialize (IHWF1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2).
          replace (S n + S n0) with (S (S n + n0)) by lia.
          eapply pfn_absorb. eassumption. eassumption. rewrite <- app_assoc. assumption.
        * symmetry in HPD2. apply Permutation_rel_singleton_nil in HPD2 as (_ & Hcontra). discriminate.
        * rewrite HPD2 in H. apply Permutation_rel_split_last in H.
          destruct H as [[HPF1 HPF2] | [F1 [F2 [HPF1 [HPF2 HPF3]]]]]; try discriminate.
          assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ ([!]u) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 c G (D1 ++ [shift_typ 0 b ([!]u)]) D1' D' F2).
          assert (HPD'1: D1 ++ [shift_typ 0 b ([!]u)] ≡[P] D1' ++ [shift_typ 0 b ([!] u)]). {rewrite HPE2. reflexivity. }
          eapply pfn_bang in HG1. 2: {reflexivity. }
          specialize (IHWF1 HPD'1 HPF2 HPG1 HPG2 HG1 HG2).
          replace (S n + S n0) with (S (S n + n0)) by lia.
          eapply pfn_one. 2: {rewrite HPF1, HPF3, app_assoc. reflexivity. } assumption.
        * rewrite HPD2 in H. apply Permutation_rel_split_last in H.
          destruct H as [[HPF1 HPF2] | [F1 [F2 [HPF1 [HPF2 HPF3]]]]]; try discriminate.
          assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ ([!]u) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 c G (D1 ++ [shift_typ 0 b ([!]u)]) D1' (D' ++ [t] ++ [u0]) (F1 ++ [t] ++ [u0])).
          assert (HPD'1: D1 ++ [shift_typ 0 b ([!]u)] ≡[P] D1' ++ [shift_typ 0 b ([!]u)]). {rewrite HPE2. reflexivity. }
          assert (HPD'2: D' ++ [t] ++ [u0] ≡[P] (F1 ++ [t] ++ [u0]) ++ [shift_typ 0 b (dual ([!]u))]). {rewrite HPF2, <- HPF3. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. }
          eapply pfn_bang in HG1. 2: {reflexivity. }
          specialize (IHWF1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2).
          replace (S n + S n0) with (S (S n + n0)) by lia.
          eapply pfn_tensor.
        ...
        * rewrite HPD2, app_assoc in H. apply Permutation_rel_split_last in H.
          ...
          -- assert (HM1: S n + n1 < S n + S (n1 + n2)) by lia.
             specialize (IHWF _ HM1 _ _ ([!]u) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
             eapply pfn_bang in HG1. 2: {reflexivity. }
             specialize (IHWF1 c G (D1 ++ [shift_typ 0 b ([!]u)]) D1' (D0 ++ [t]) (I1 ++ [t])).
             assert (HPD'1: D1 ++ [shift_typ 0 b ([!]u)] ≡[P] D1' ++ [shift_typ 0 b ([!]u)]). {rewrite HPE2. reflexivity. }
             assert (HPD'2: D0 ++ [t] ≡[P] (I1 ++ [t]) ++ [shift_typ 0 b (dual ([!]u))]). {rewrite HPI1. apply Permutation_rel_assoc_swap. }
             specialize (IHWF1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2_1).
             replace (S n + S (n1 + n2)) with (S ((S n + n1) + n2)) by lia.
             eapply pfn_par.
             3: {assert (D1' ++ D2' ≡[P] (D1' ++ I1) ++ D2 ++ [t_par t u0]). {rewrite HPF1 , HPF3, <- HPI2. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. } eassumption. }
             ++ rewrite <- app_assoc. assumption.
             ++ assumption.
          -- assert (HM1: S n + n2 < S n + S (n1 + n2)) by lia.
             specialize (IHWF _ HM1 _ _ ([!]u) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
             specialize (IHWF1 c G (D1 ++ [shift_typ 0 b ([!]u)]) D1' (D2 ++ [u0]) (I2 ++ [u0])).
             assert (HPD'1: D1 ++ [shift_typ 0 b ([!] u)] ≡[P] D1' ++ [shift_typ 0 b ([!]u)]). {rewrite HPE2. reflexivity. }
             assert (HPD'2: D2 ++ [u0] ≡[P] (I2 ++ [u0]) ++ [shift_typ 0 b (dual ([!]u))]). {rewrite HPI1. apply Permutation_rel_assoc_swap. }
             eapply pfn_bang in HG1. 2: {reflexivity. }
             specialize (IHWF1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2_2).
             replace (S n + S (n1 + n2)) with (S (n1 + (S n + n2))) by lia.
             eapply pfn_par.
             3: {assert (D1' ++ D2' ≡[P] D0 ++ (D1' ++ I2) ++ [t_par t u0]). {rewrite HPF1, HPF3, <- HPI2. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. } eassumption. }
             ++ assumption.
        ...
        * rewrite HPD2 in H. apply Permutation_rel_split_last in H.
          ...
          assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ ([!]u) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 c (G ++ [t]) (D1 ++ [shift_typ 0 b ([!]u)]) D1' D0 F2); listSimpl.
          assert (HPD'1: D1 ++ [shift_typ 0 b ([!]u)] ≡[P] D1' ++ [shift_typ 0 b ([!]u)]). {rewrite HPE2. reflexivity. }
          eapply pfn_bang in HG1. 2: {reflexivity. }
          assert (HG'1: S n ⊣ c, G ++ [t], D1 ++ [shift_typ 0 b ([!]u)] ⊢pf). {eapply pfn_weakening. reflexivity. pfn_wf_ctx_solver. assumption. }
          assert (HPG'1: G ++ [t] ≡[P] G ++ [t]) by reflexivity.
          specialize (IHWF1 HPD'1 HPF2 HPG'1 HPG'1 HG'1 HG2).
          replace (S n + S n0) with (S (S n + n0)) by lia.
          eapply pfn_bang. 2: {rewrite HPF1, HPF3, app_assoc. reflexivity. } assumption.
        * symmetry in HPD2. apply Permutation_rel_singleton_nil in HPD2 as (-> & HPD2).
          ...
          (* specialize (IHu' b c (G ++ [shift_typ 0 b u]) G D1 [shift_typ 0 b (dual u)]); listSimpl. *)
          (* assert (HPD'1: D1 ≡[P] D1) by reflexivity. *)
          (* assert (HPD'2: [shift_typ 0 b (dual u)] ≡[P] [shift_typ 0 b (dual u)]) by reflexivity. *)
          (* assert (HPG'1: G ++ [shift_typ 0 b u] ≡[P] G ++ [shift_typ 0 b u]) by reflexivity. *)
        ...
        * rewrite HPD2 in H0. apply Permutation_rel_split_last in H0.
          ...
          assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ ([!]u) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 c G D D1' (D0 ++ [typ_subst 0 u0 t]) (F2 ++ [typ_subst 0 u0 t])); listSimpl.
          assert (HPD'2: D0 ++ [typ_subst 0 u0 t] ≡[P] (F2 ++ [typ_subst 0 u0 t]) ++ [shift_typ 0 b (dual ([!]u))]). {rewrite HPF2. apply Permutation_rel_assoc_swap. }
          eapply pfn_bang in HG1. 2: {rewrite <- HPE2. apply HPD1. }
          specialize (IHWF1 HPD1 HPD'2 HPG1 HPG2 HG1 HG2).
          replace (S n + S n0) with (S (S n + n0)) by lia.
          eapply pfn_forall. eassumption. rewrite app_assoc in IHWF1. eassumption. rewrite HPF1, HPF3, app_assoc. reflexivity.
        * rewrite HPD2 in H. apply Permutation_rel_split_last in H.
          destruct H as [[HPF1 HPF2] |[F1 [F2 [HPF1 [HPF2 HPF3]]]]]; try discriminate.
          assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ ([!]u) eq_refl (1 + b)) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 (1 + c) (shift_ctx 0 1 G) (shift_ctx 0 1 D) (shift_ctx 0 1 D1') (shift_ctx 0 1 D0 ++ [u0]) (shift_ctx 0 1 F2 ++ [u0])); listSimpl.
          assert (HSD1: shift_ctx 0 1 D ≡[P] shift_ctx 0 1 D1' ++ shift_ctx 0 (1 + b) [[!] u]).
          {rewrite HPD1, shift_ctx_app. simpl; repeat rewrite shift_typ_plus; reflexivity. }
          assert (HSD2: shift_ctx 0 1 D0 ++ [u0] ≡[P] (shift_ctx 0 1 F2 ++ [u0]) ++ shift_ctx 0 (1 + b) [dual ([!]u)]).
          {rewrite HPF2, shift_ctx_app, Permutation_rel_assoc_swap. simpl. repeat rewrite shift_typ_plus. reflexivity. }
          assert (HSG: shift_ctx 0 1 G ≡[P] shift_ctx 0 1 G) by reflexivity.
          eapply pfn_bang in HG1. 2: {rewrite <- HPE2. apply HPD1. }
          eapply (pfn_shift_inv 0 1) in HG1; try lia.
          replace (c + 1) with (1 + c) in * by lia.
          specialize (IHWF1 HSD1 HSD2 HSG HSG HG1 HG2).
          replace (S n + S n0) with (S (S n + n0)) by lia.
          eapply pfn_exists.
          -- rewrite app_assoc, <- shift_ctx_app in IHWF1. apply IHWF1.
          -- rewrite HPF1, HPF3, app_assoc. reflexivity.
      + assert (HM1 : n + m < S n + m) by lia.
        specialize (IHWF _ HM1 _ _ ([!]u) eq_refl b) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        ...
        specialize (IHWF1 HPE2 HPD2 HPG'1 HPG'1 HG1 HG'1).
        replace (S n + m) with (S (n + m)) by lia.
        eapply pfn_bang. eapply IHWF1. rewrite HPE1, HPE3. apply Permutation_rel_assoc_swap.
    - symmetry in HPD1. apply Permutation_rel_singleton_nil in HPD1 as (_ & Hcontra). discriminate.
    - rewrite HPD1 in H0. apply Permutation_rel_split_last in H0. 
      destruct H0 as [[HPE1 HPE2] | [E1 [E2 [HPE1 [HPE2 HPE3]]]]]; try discriminate.
      assert (HM1 : n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ ([!]u) eq_refl b) as (IHWF1 & IWHF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G (D1 ++ [typ_subst 0 u0 t]) (E2 ++ [typ_subst 0 u0 t]) D2 D2').
      assert (HPD'1: D1 ++ [typ_subst 0 u0 t] ≡[P] (E2 ++ [typ_subst 0 u0 t]) ++ [shift_typ 0 b ([!]u)]). {rewrite HPE2. apply Permutation_rel_assoc_swap. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2).
      replace (S n + m) with (S (n + m)) by lia.
      eapply pfn_forall. eassumption. 2: {rewrite HPE1, HPE3. apply Permutation_rel_assoc_swap. }
      eapply pfn_perm_rel. reflexivity. apply Permutation_rel_assoc_swap. assumption.
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H. 
      destruct H as [[HPE1 HPE2] | [E1 [E2 [HPE1 [HPE2 HPE3]]]]]; try discriminate.
      assert (HM1 : n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ ([!]u) eq_refl (1 + b)) as (IHWF1 & IWHF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 (1 + c) (shift_ctx 0 1 G) (shift_ctx 0 1 D1 ++ [u0]) (shift_ctx 0 1 E2 ++ [u0]) (shift_ctx 0 1 D2) (shift_ctx 0 1 D2')); listSimpl.
      assert (HSD1: shift_ctx 0 1 D1 ++ [u0] ≡[P] (shift_ctx 0 1 E2 ++ [u0]) ++ shift_ctx 0 (1 + b) [[!]u]).
      {rewrite HPE2, shift_ctx_app, Permutation_rel_assoc_swap. simpl; repeat rewrite shift_typ_plus. reflexivity. }
      assert (HSD2: shift_ctx 0 1 D2 ≡[P] shift_ctx 0 1 D2' ++ shift_ctx 0 (1 + b) [dual ([!]u)]).
      {rewrite HPD2, shift_ctx_app. simpl; repeat rewrite shift_typ_plus. reflexivity. }
      assert (HSG: shift_ctx 0 1 G ≡[P] shift_ctx 0 1 G) by reflexivity.
      eapply (pfn_shift_inv 0 1) in HG2; try lia.
      replace (c + 1) with (1 + c) in * by lia.
      specialize (IHWF1 HSD1 HSD2 HSG HSG HG1 HG2).
      replace (S n + m) with (S (n + m)) by lia.
      eapply pfn_exists.
      2: {rewrite HPE1, HPE3. eapply Permutation_rel_assoc_swap. }
      eapply pfn_perm_rel. reflexivity. rewrite shift_ctx_app. apply Permutation_rel_assoc_swap. assumption.
  Qed.
  ...
  Qed.
  Lemma cut_admissibility : forall n m u b, cut_admissibility_norm n m u b /\ ((forall n' m' b, cut_admissibility_norm n' m' u b) -> cut_admissibility_bang n m u b) /\ ((forall n' m' b, cut_admissibility_norm n' m' u b) -> cut_admissibility_ques n m u b).
  Proof. 
    intros n m u.
    remember (n + m) as o. revert n m u Heqo.
    induction (lt_wf o). rename H into IHY. rename H0 into IHWF. rename x into n'.
    intros n m u. revert n' n m IHY IHWF.
    induction u; intros.
    - admit.
    - admit.
    - eapply cut_admissibility_tensor; eassumption.
    - admit.
    - admit.
    - admit.
      ...
      (* pose proof (cut_admissibility_bang_case (dual u) n m b IHu'). *)
      (* auto. *)
    - 
    - intros. eapply cut_admissibility_base; eassumption.
    - intros. eapply cut_admissibility_var; eassumption.
    ...
    - apply (cut_admissibility_dual ([?] u)).
      ...
      auto.
    - intros. eapply cut_admissibility_forall; eassumption.
    - apply (cut_admissibility_dual ([exists] u)).
      intros.
      specialize (proj1 (cut_admissibility_dual _) IHu) as IHu'. clear IHu.
      pose proof (cut_admissibility_forall (dual u) n m IHu').
      auto.
  Qed.
  
  
Lemma cut_admissibility_tensor_norm :
  forall u1 u2 x n m b
    ...
    (IHWF : ∀ y : nat,
        y < x
         → ∀ n m b : nat,
             y = n + m
             → (∀ n0 m0 b0 : nat,
                  cut_admissibility_norm n0 m0 u1 b0
                  ∧ ((∀ n' m' b1 : nat, cut_admissibility_norm n' m' u1 b1) → cut_admissibility_bang n0 m0 u1 b0)
                    ∧ ((∀ n' m' b1 : nat, cut_admissibility_norm n' m' u1 b1) → cut_admissibility_ques n0 m0 u1 b0))
               → (∀ n0 m0 b0 : nat,
                    cut_admissibility_norm n0 m0 u2 b0
                    ∧ ((∀ n' m' b1 : nat, cut_admissibility_norm n' m' u2 b1) → cut_admissibility_bang n0 m0 u2 b0)
                      ∧ ((∀ n' m' b1 : nat, cut_admissibility_norm n' m' u2 b1) → cut_admissibility_ques n0 m0 u2 b0))
                 → cut_admissibility_norm n m (u1 ⊗ u2) b
                   ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' (u1 ⊗ u2) b0) → cut_admissibility_bang n m (u1 ⊗ u2) b)
                     ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' (u1 ⊗ u2) b0) → cut_admissibility_ques n m (u1 ⊗ u2) b))
    (Heqo : x = n + m)
    (IHu1 : ∀ n m b,
        cut_admissibility_norm n m u1 b
        ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u1 b0) → cut_admissibility_bang n m u1 b)
        ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u1 b0) → cut_admissibility_ques n m u1 b))
    (IHu2 : ∀ n m b : nat,
           cut_admissibility_norm n m u2 b
           ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u2 b0) → cut_admissibility_bang n m u2 b)
             ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u2 b0) → cut_admissibility_ques n m u2 b)),
    cut_admissibility_norm n m (u1 ⊗ u2) b.
Proof.
    intros. unfold cut_admissibility_norm, cut_admissibility_t.
    intros c G D1 D1' D2 D2' HPD1 HPD2 HPG1 HPG2 HG1 HG2.
    destruct HG1; subst.
    - eapply cut_admissibility_id1'; eassumption.
    - assert (HM1: n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ b eq_refl IHu1 IHu2) as (IHWF1 & IHWF2 & IHWF3); unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G' (D ++ [t]) (D1' ++ [t]) D2 D2'). listSimpl.
      assert (D ++ [t] ≡[P] (D1' ++ [t]) ++ shift_ctx 0 b [u1 ⊗ u2]). {rewrite HPD1. clear_permrel. apply Permutation_rel_assoc_swap. }
      specialize (IHWF1 H1 HPD2 HPG1 HPG2 HG1 HG2).
    ...
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H.
      ...
      + injection HPE1. intros. subst. clear HPE1.
        destruct HG2.
        * eapply pfn_tensor in HG1. 2: {reflexivity. }.
           eapply cut_admissibility_id2'. eassumption. rewrite dual_involutive. eassumption.
           eapply pfn_perm_rel. reflexivity. rewrite HPD1, HPE2. reflexivity. apply HG1.
        * assert (HM1: (S n) + n0 < S n + S n0) by lia.
           specialize (IHWF _ HM1 _ _ b eq_refl IHu1 IHu2) as (IHWF1 & IHWF2 & IHWF3); unfold_admissibilityH IHWF1.
           specialize (IHWF1 c G' D D1' (D0 ++ [t]) (D2' ++ [t])); listSimpl.
        ...
        * rewrite HPD2 in H. apply Permutation_rel_split_last in H.
          ...
          assert (HM1: S n + n0 < S n + S n0) by lia.
          eapply pfn_tensor in HG1. 2: {rewrite <- HPE2. eassumption. }
          specialize (IHWF _ HM1 _ _ (b + 1) eq_refl IHu1 IHu2) as (IHWF1 & IWHF2 & IHWFF3); unfold_admissibilityH IHWF1.
          apply (pfn_shift_inv 0 1) in HG1; try lia.
          specialize (IHWF1 (1 + c) (shift_ctx 0 1 G) (shift_ctx 0 1 D) (shift_ctx 0 1 D1') (shift_ctx 0 1 D1 ++ [u]) (shift_ctx 0 1 F2 ++ [u])). listSimpl.
          assert (HS1 : shift_ctx 0 1 D ≡[P] shift_ctx 0 1 D1' ++ shift_ctx 0 (b + 1) [u1 ⊗ u2]).
          {rewrite HPD1, shift_ctx_app. simpl. repeat rewrite shift_typ_plus. replace (1 + b) with (b + 1) by lia. reflexivity. }
          ...
          assert (HSG : shift_ctx 0 1 G ≡[P] shift_ctx 0 1 G) by reflexivity.
          replace (c + 1) with (1 + c) in * by lia.
          specialize (IHWF1 HS1 HS2 HSG HSG HG1 HG2). destruct IHWF1 as (k & IHWF1).
          exists (S k). eapply pfn_exists.
          -- rewrite app_assoc, <- shift_ctx_app in IHWF1. eapply IHWF1.
          -- rewrite HPF1, HPF3, app_assoc. reflexivity.
      + subst.
    ...
    - rewrite HPD1, app_assoc in H. apply Permutation_rel_split_last in H.
      ...
      + assert (HM1: n1 + m < S (Nat.max n1 n2) + m) by lia.
        specialize (IHWF _ HM1 _ _ b eq_refl IHu1 IHu2) as (IHWF1 & IHWF2 & IHWF3). unfold cut_admissibility_norm, cut_admissibility_t in IHWF1.
        specialize (IHWF1 c G (D1 ++ [t]) (F1 ++ [t]) D2 D2'); listSimpl.
        assert (HPD'1: D1 ++ [t] ≡[P] (F1 ++ [t]) ++ [shift_typ 0 b (u1 ⊗ u2)]). {rewrite HPF1. apply Permutation_rel_assoc_swap. }
    ...
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H.
      destruct H as [[HPE1 HPE2] | [E1 [E2 [HPE1 [HPE2 HPE3]]]]]; try discriminate.
      subst. assert (HM1: n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ (b + 1) eq_refl IHu1 IHu2) as (IHWF1 & IHWF2 & IHWF3); unfold_admissibilityH IHWF1.
      specialize (IHWF1 (1 + c) (shift_ctx 0 1 G) (shift_ctx 0 1 D1 ++ [u]) (shift_ctx 0 1 E1 ++ [u]) (shift_ctx 0 1 D2) (shift_ctx 0 1 D2')); listSimpl.
      eapply (pfn_shift_inv 0 1) in HG2; try lia.
      assert (HS1 : shift_ctx 0 1 D1 ++ [u] ≡[P] (shift_ctx 0 1 E1 ++ [u]) ++ shift_ctx 0 (b + 1) [u1 ⊗ u2]).
      {rewrite HPE2, HPE3, shift_ctx_app, Permutation_rel_assoc_swap. simpl. repeat rewrite shift_typ_plus. replace (1 + b) with (b + 1) by lia. reflexivity. }
...
Lemma cut_admissibility_tensor_bang :
  ...
  Proof.
    ...
    - rewrite HPG2 in H0. apply Permutation_rel_split_last in H0.
      ...
      + 
        subst.
        assert (HM1: n + n0 < n + S n0) by lia.
        specialize (IHWF _ HM1 _ _ b eq_refl IHu1 IHu2) as (IHWF1 & IHWF2 & IHWF3). specialize (IHWF2 HC). unfold_admissibilityH IHWF2.
        specialize (IHWF2 c G1 G' D1 (D ++ [shift_typ 0 b (dual (u1 ⊗ u2))])); listSimpl.
        assert (HPD'1: D ++ [shift_typ 0 b (dual (u1 ⊗ u2))] ≡[P] D ++ [shift_typ 0 b (dual (u1 ⊗ u2))]) by reflexivity.
        specialize (IHWF2 HPD1 HPD'1 HPG1 HPG2 HG1 HG2) as (k & IHWF2).
        unfold_admissibilityH HC. specialize (HC n k b c G1 D1 [] (D ++ [shift_typ 0 b (dual (u1 ⊗ u2))]) D); listSimpl.
    ...
    - pose proof IHWF as IHWF'.
      ...
      assert (HPD'1: D0 ++ [t] ≡[P] D0 ++ [t]) by reflexivity.
      specialize (IHWF2 HPD1 HPD'1 HPG1 HPG2 HG1 HG2_1) as (k1 & IHWF2).
      specialize (IHWF' _ HM2 _ _ b eq_refl IHu1 IHu2) as (IHWF'1 & IHWF'2 & IHWF'3). specialize (IHWF'2 HC); unfold_admissibilityH IHWF'2.
      specialize (IHWF'2 c G1 G D1 (D2 ++ [u])); listSimpl.
      assert (HPD'2 : D2 ++ [u] ≡[P] D2 ++ [u]) by reflexivity.
      specialize (IHWF'2 HPD1 HPD'2 HPG1 HPG2 HG1 HG2_2) as (k2 & IHWF'2).
    ...
    - assert (HM1: n + n0 < n + S n0) by lia.
      ...
      exists (S k). eapply pfn_forall. eassumption. apply IHWF2. assumption.
    - assert (HM1 : n + n0 < n + S n0) by lia.
      specialize (IHWF _ HM1 _ _ (b + 1) eq_refl IHu1 IHu2) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF2 HC); unfold_admissibilityH IHWF2.
      eapply (pfn_shift_inv 0 1) in HG1; try lia.
      specialize (IHWF2 (1 + c) (shift_ctx 0 1 G1) (shift_ctx 0 1 G) (shift_ctx 0 1 D1) (shift_ctx 0 1 D0 ++ [u])); listSimpl.
      assert (HS1 : shift_ctx 0 1 D1 ≡[P] shift_ctx 0 (b + 1) [u1 ⊗ u2]).
      {rewrite HPD1. simpl; repeat rewrite shift_typ_plus. replace (1 + b) with (b + 1) by lia. reflexivity. }
    ...
    forall u1 u2 x n m b
      ...
      (IHWF : ∀ y : nat,
          ...
          → ∀ n m b : nat,
            ...
            → (∀ n0 m0 b0 : nat,
                  cut_admissibility_norm n0 m0 u1 b0
                  ∧ ((∀ n' m' b1 : nat, cut_admissibility_norm n' m' u1 b1)
                     → cut_admissibility_bang n0 m0 u1 b0)
                  ∧ ((∀ n' m' b1 : nat, cut_admissibility_norm n' m' u1 b1)
                     → cut_admissibility_ques n0 m0 u1 b0))
            → (∀ n0 m0 b0 : nat,
                  cut_admissibility_norm n0 m0 u2 b0
                  ∧ ((∀ n' m' b1 : nat, cut_admissibility_norm n' m' u2 b1)
                     → cut_admissibility_bang n0 m0 u2 b0)
                  ∧ ((∀ n' m' b1 : nat, cut_admissibility_norm n' m' u2 b1)
                     → cut_admissibility_ques n0 m0 u2 b0))
            → cut_admissibility_norm n m (u1 ⊗ u2) b
              ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' (u1 ⊗ u2) b0)
                 → cut_admissibility_bang n m (u1 ⊗ u2) b)
              ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' (u1 ⊗ u2) b0)
                 → cut_admissibility_ques n m (u1 ⊗ u2) b))
      (Heqo : x = n + m)
      (IHu1 : ∀ n m b : nat,
          cut_admissibility_norm n m u1 b
          ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u1 b0)
             → cut_admissibility_bang n m u1 b)
          ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u1 b0)
             → cut_admissibility_ques n m u1 b))
      (IHu2 : ∀ n m b: nat,
          cut_admissibility_norm n m u2 b
          ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u2 b0)
             → cut_admissibility_bang n m u2 b)
          ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u2 b0)
             → cut_admissibility_ques n m u2 b))
      (HC: forall n m b0, cut_admissibility_norm n m (u1 ⊗ u2) b0), cut_admissibility_ques n m (u1 ⊗ u2) b.
  Proof.
    intros. unfold cut_admissibility_ques, cut_admissibility_t.
    intros c G1 G2 D1 D2 HPD1 HPD2 HPG1 HPG2 HG1 HG2. listSimpl. 
    ...
    - rewrite HPG1 in H0. apply Permutation_rel_split_last in H0.
      ...
      + 
        subst.
        assert (HM1: n + m < S n + m) by lia.
        specialize (IHWF _ HM1 _ _ b eq_refl IHu1 IHu2) as (IHWF1 & IHWF2 & IHWF3). specialize (IHWF3 HC). unfold_admissibilityH IHWF3.
        specialize (IHWF3 c G' G2 (D ++ [shift_typ 0 b (u1 ⊗ u2)]) D2); listSimpl.
        assert (HPD'1: D ++ [shift_typ 0 b (u1 ⊗ u2)] ≡[P] D ++ [shift_typ 0 b (u1 ⊗ u2)]) by reflexivity.
        specialize (IHWF3 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF3).
        unfold_admissibilityH HC. specialize (HC k m b c G2 (D ++ [shift_typ 0 b(u1 ⊗ u2)]) D D2 []); listSimpl.
    ...
    - pose proof IHWF as IHWF'.
      ...
      assert (HPD'1: D1 ++ [t] ≡[P] D1 ++ [t]) by reflexivity.
      specialize (IHWF3 HPD'1 HPD2 HPG1 HPG2 HG1_1 HG2) as (k1 & IHWF3).
      specialize (IHWF' _ HM2 _ _ b eq_refl IHu1 IHu2) as (IHWF'1 & IHWF'2 & IHWF'3). specialize (IHWF'3 HC); unfold_admissibilityH IHWF'3.
      specialize (IHWF'3 c G G2 (D0 ++ [u]) D2); listSimpl.
      assert (HPD'2 : D0 ++ [u] ≡[P] D0 ++ [u]) by reflexivity.
      specialize (IHWF'3 HPD'2 HPD2 HPG1 HPG2 HG1_2 HG2) as (k2 & IHWF'3).
      exists (S (Nat.max k1 k2)). eapply pfn_par; try eassumption.
    ...
    - assert (HM1: n + m < S n + m) by lia.
      ...
      exists (S k). eapply pfn_forall. eassumption. apply IHWF3. assumption.
    - assert (HM1 : n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ (1 + b) eq_refl IHu1 IHu2) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF3 HC); unfold_admissibilityH IHWF3.
      specialize (IHWF3 (1 + c) (shift_ctx 0 1 G) (shift_ctx 0 1 G2) (shift_ctx 0 1 D1 ++ [u]) (shift_ctx 0 1 D2)); listSimpl.
      assert (HSD1: shift_ctx 0 1 D1 ++ [u] ≡[P] shift_ctx 0 1 D1 ++ [u]) by reflexivity.
      assert (HSD2: shift_ctx 0 1 D2 ≡[P] shift_ctx 0 (1 + b) [dual (u1 ⊗ u2)]).
      {rewrite HPD2. simpl; repeat rewrite shift_typ_plus. reflexivity. }
  ...
  (IHu1 : ∀ n m b : nat,
           cut_admissibility_norm n m u1 b
           ∧ ((∀ n' m' b : nat, cut_admissibility_norm n' m' u1 b) → cut_admissibility_bang n m u1 b)
             ∧ ((∀ n' m' b : nat, cut_admissibility_norm n' m' u1 b) → cut_admissibility_ques n m u1 b))
  (IHu2 : ∀ n m b : nat,
           cut_admissibility_norm n m u2 b
           ∧ ((∀ n' m' b : nat, cut_admissibility_norm n' m' u2 b) → cut_admissibility_bang n m u2 b)
             ∧ ((∀ n' m' b : nat, cut_admissibility_norm n' m' u2 b) → cut_admissibility_ques n m u2 b)),
    cut_admissibility_norm n m (u1 ⊗ u2) b
    ∧ ((∀ n' m' b : nat, cut_admissibility_norm n' m' (u1 ⊗ u2) b) → cut_admissibility_bang n m (u1 ⊗ u2) b)
      ∧ ((∀ n' m' b : nat, cut_admissibility_norm n' m' (u1 ⊗ u2) b) → cut_admissibility_ques n m (u1 ⊗ u2) b).
  Proof.
    intros u1 u2 n m b.
    remember (n + m) as o.
    revert n m b Heqo.
    ...
    forall u x n m b
      ...
      (IHWF : ∀ y : nat,
         ...
         → ∀ n m b : nat,
             y = n + m
             → (∀ n0 m0 b0 : nat,
                    cut_admissibility_norm n0 m0 u b0
                    ∧ ((∀ n' m' b1 : nat, cut_admissibility_norm n' m' u b1) → cut_admissibility_bang n0 m0 u b0)
                      ∧ ((∀ n' m' b1 : nat, cut_admissibility_norm n' m' u b1) → cut_admissibility_ques n0 m0 u b0))
                 → cut_admissibility_norm n m ([!] u) b
                   ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' ([!] u) b0) → cut_admissibility_bang n m ([!] u) b)
                     ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' ([!] u) b0) → cut_admissibility_ques n m ([!] u) b))
      (Heqo : x = n + m)
  ...
  Proof.
    ...
    - eapply cut_admissibility_id1'; eassumption.
    - subst. assert (HM1: n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G' (D ++ [t]) (D1' ++ [t]) D2 D2').
      assert (HPD'1: D ++ [t] ≡[P] (D1' ++ [t]) ++ [shift_typ 0 b ([!] u)]). {rewrite HPD1. apply Permutation_rel_assoc_swap. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2).
      destruct IHWF1 as (k & IHWF1).
    ...
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H.
      ...
      + injection HPE1. intros; subst. clear HPE1.
        destruct HG2.
        * eapply cut_admissibility'_aux_id2. eassumption. rewrite dual_shift_typ_comm. rewrite dual_involutive. eassumption.
          eapply pfn_bang. eassumption. rewrite <- HPE2. assumption.
        * assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 c G' (D1 ++ [shift_typ 0 b ([!]u)]) D1' (D0 ++ [t]) (D2' ++ [t])).
        ...
        * rewrite HPD2 in H. apply Permutation_rel_split_last in H.
          destruct H as [[HPF1 HPF2] |[F1 [F2 [HPF1 [HPF2 HPF3]]]]]; try discriminate.
          assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ (1 + b) eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 (1 + c) (shift_ctx 0 1 G) (shift_ctx 0 1 D) (shift_ctx 0 1 D1') (shift_ctx 0 1 D0 ++ [u0]) (shift_ctx 0 1 F2 ++ [u0])); listSimpl.
          assert (HSD1: shift_ctx 0 1 D ≡[P] shift_ctx 0 1 D1' ++ shift_ctx 0 (1 + b) [[!] u]).
          {rewrite HPD1, shift_ctx_app. simpl; repeat rewrite shift_typ_plus; reflexivity. }
          assert (HSD2: shift_ctx 0 1 D0 ++ [u0] ≡[P] (shift_ctx 0 1 F2 ++ [u0]) ++ shift_ctx 0 (1 + b) [dual ([!]u)]).
          ...
          eapply (pfn_shift_inv 0 1) in HG1; try lia.
          replace (c + 1) with (1 + c) in * by lia.
          specialize (IHWF1 HSD1 HSD2 HSG HSG HG1 HG2). destruct IHWF1 as (k & IHWF1).
          exists (S k). eapply pfn_exists.
          -- rewrite app_assoc, <- shift_ctx_app in IHWF1. apply IHWF1.
          -- rewrite HPF1, HPF3, app_assoc. reflexivity.
      + assert (HM1 : n + m < S n + m) by lia.
    ...
    - rewrite HPD1 in H0. apply Permutation_rel_split_last in H0. 
      destruct H0 as [[HPE1 HPE2] | [E1 [E2 [HPE1 [HPE2 HPE3]]]]]; try discriminate.
      assert (HM1 : n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IWHF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G (D1 ++ [typ_subst 0 u0 t]) (E2 ++ [typ_subst 0 u0 t]) D2 D2').
      assert (HPD'1: D1 ++ [typ_subst 0 u0 t] ≡[P] (E2 ++ [typ_subst 0 u0 t]) ++ [shift_typ 0 b ([!]u)]). {rewrite HPE2. apply Permutation_rel_assoc_swap. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
    ...
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H. 
      destruct H as [[HPE1 HPE2] | [E1 [E2 [HPE1 [HPE2 HPE3]]]]]; try discriminate.
      assert (HM1 : n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ (1 + b) eq_refl IHu) as (IHWF1 & IWHF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 (1 + c) (shift_ctx 0 1 G) (shift_ctx 0 1 D1 ++ [u0]) (shift_ctx 0 1 E2 ++ [u0]) (shift_ctx 0 1 D2) (shift_ctx 0 1 D2')); listSimpl.
      assert (HSD1: shift_ctx 0 1 D1 ++ [u0] ≡[P] (shift_ctx 0 1 E2 ++ [u0]) ++ shift_ctx 0 (1 + b) [[!]u]).
      {rewrite HPE2, shift_ctx_app, Permutation_rel_assoc_swap. simpl; repeat rewrite shift_typ_plus. reflexivity. }
      assert (HSD2: shift_ctx 0 1 D2 ≡[P] shift_ctx 0 1 D2' ++ shift_ctx 0 (1 + b) [dual ([!]u)]).
  ...
  Lemma cut_admissibility_bang_bang:
    forall u x n m b
      ...
      (IHu : ∀ n m b : nat,
          ...
          ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u b0) → cut_admissibility_bang n m u b)
            ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u b0) → cut_admissibility_ques n m u b))
      (HC: forall n m b, cut_admissibility_norm n m ([!] u) b),
      cut_admissibility_bang n m ([!] u) b.
  Proof.
    intros. unfold cut_admissibility_bang, cut_admissibility_t.
    intros c G1 G2 D1 D2 HPD1 HPD2 HPG1 HPG2 HG1 HG2. listSimpl. 
    destruct HG2; subst.
    ...
    - rewrite HPG2 in H0. apply Permutation_rel_split_last in H0.
      ...
      + 
        subst.
        assert (HM1: n + n0 < n + S n0) by lia.
        specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). specialize (IHWF2 HC). unfold_admissibilityH IHWF2.
        specialize (IHWF2 c G1 G' D1 (D ++ [shift_typ 0 b (dual ([!] u))])); listSimpl.
        assert (HPD'1: D ++ [shift_typ 0 b (dual ([!] u))] ≡[P] D ++ [shift_typ 0 b (dual ([!]u))]) by reflexivity.
        specialize (IHWF2 HPD1 HPD'1 HPG1 HPG2 HG1 HG2) as (k & IHWF2).
        unfold_admissibilityH HC. specialize (HC n k b c G1 D1 [] (D ++ [shift_typ 0 b (dual ([!]u))]) D); listSimpl.
        specialize (HC HPD1 HPD'1 HPG1 HPG1 HG1 IHWF2) as (k2 & HC).
        eexists; eauto.
      + assert (HM1 : n + n0 < n + S n0) by lia.
        specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF2 HC); unfold_admissibilityH IHWF2.
    ...
    - pose proof IHWF as IHWF'.
      ...
      assert (HPD'1: D0 ++ [t] ≡[P] D0 ++ [t]) by reflexivity.
      specialize (IHWF2 HPD1 HPD'1 HPG1 HPG2 HG1 HG2_1) as (k1 & IHWF2).
      specialize (IHWF' _ HM2 _ _ b eq_refl IHu) as (IHWF'1 & IHWF'2 & IHWF'3). specialize (IHWF'2 HC); unfold_admissibilityH IHWF'2.
    ...
    - assert (HM1: n + n0 < n + S n0) by lia.
      ...
      assert (HPD'1 : D0 ≡[P] D0) by reflexivity.
      assert (HPG'1 : G1 ++ [t] ≡[P] G1 ++ [t]) by reflexivity.
      assert (HPG'2 : G ++ [t] ≡[P] (G1 ++ [t]) ++ [shift_typ 0 b (dual ([!]u))]) by (rewrite HPG2; apply Permutation_rel_assoc_swap).
      assert (HG'1 : n ⊣ c, (G1 ++ [t]), D1 ⊢pf).
      {eapply pfn_weakening. reflexivity. pfn_wf_ctx_solver. assumption. }
      specialize (IHWF2 HPD1 HPD'1 HPG'1 HPG'2 HG'1 HG2) as (k & IHWF2).
      exists (S k). eapply pfn_bang. 2: {eassumption. } assumption.
    ...
    - assert (HM1: n + n0 < n + S n0) by lia.
      specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF2 HC); unfold_admissibilityH IHWF2.
      specialize (IHWF2 c G1 G D1 (D0 ++ [typ_subst 0 u0 t])); listSimpl.
      assert (HPD'1: D0 ++ [typ_subst 0 u0 t] ≡[P] D0 ++[typ_subst 0 u0 t]) by reflexivity.
      specialize (IHWF2 HPD1 HPD'1 HPG1 HPG2 HG1 HG2) as (k & IHWF2).
      exists (S k). eapply pfn_forall. eassumption. apply IHWF2. assumption.
    - assert (HM1: n + n0 < n + S n0) by lia.
      specialize (IHWF _ HM1 _ _ (1 + b) eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF2 HC); unfold_admissibilityH IHWF2.
      specialize (IHWF2 (1 + c) (shift_ctx 0 1 G1) (shift_ctx 0 1 G) (shift_ctx 0 1 D1) (shift_ctx 0 1 D0 ++ [u0])); listSimpl.
      assert (HSD1: shift_ctx 0 1 D1 ≡[P] shift_ctx 0 (1 + b) [[!] u]).
      {rewrite HPD1. simpl. repeat rewrite shift_typ_plus. reflexivity. }
      assert (HSD2: shift_ctx 0 1 D0 ++ [u0] ≡[P] shift_ctx 0 1 D0 ++ [u0]) by reflexivity.
      assert (HSG1: shift_ctx 0 1 G1 ≡[P] shift_ctx 0 1 G1) by reflexivity.
      assert (HSG2: shift_ctx 0 1 G ≡[P] shift_ctx 0 1 G1 ++ shift_ctx 0 (1 + b) [dual ([!]u)]).
      {rewrite HPG2, shift_ctx_app. simpl. repeat rewrite shift_typ_plus. reflexivity. }
      eapply (pfn_shift_inv 0 1) in HG1; try lia.
      replace (c + 1) with (1 + c) in * by lia.
      specialize (IHWF2 HSD1 HSD2 HSG1 HSG2 HG1 HG2). destruct IHWF2 as (k & IHWF2).
      exists (S k). eapply pfn_exists; eassumption.
  Qed.
    ...
    forall u x n m b
      ...
      (IHu : ∀ n m b : nat,
          ...
          ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u b0) → cut_admissibility_bang n m u b)
            ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u b0) → cut_admissibility_ques n m u b))
      (HC: forall n m b, cut_admissibility_norm n m ([!] u) b),
      cut_admissibility_ques n m ([!] u) b.
  Proof.
    intros. unfold cut_admissibility_ques, cut_admissibility_t.
    intros c G1 G2 D1 D2 HPD1 HPD2 HPG1 HPG2 HG1 HG2. listSimpl. 
    destruct HG1; subst.
    ...
    - rewrite HPG1 in H0. apply Permutation_rel_split_last in H0.
      ...
      + 
        subst.
        assert (HM1: n + m < S n + m) by lia.
        specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). specialize (IHWF3 HC). unfold_admissibilityH IHWF3.
        specialize (IHWF3 c G' G2 (D ++ [shift_typ 0 b ([!] u)]) D2); listSimpl.
        assert (HPD'1: D ++ [shift_typ 0 b ([!]u)] ≡[P] D ++ [shift_typ 0 b ([!]u)]) by reflexivity.
        specialize (IHWF3 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF3).
        unfold_admissibilityH HC. specialize (HC k m b c G2 (D ++ [shift_typ 0 b ([!]u)]) D D2 []); listSimpl.
        specialize (HC HPD'1 HPD2 HPG2 HPG2 IHWF3 HG2) as (k2 & HC).
        eexists; eauto.
      + assert (HM1 : n + m < S n + m) by lia.
        specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF3 HC); unfold_admissibilityH IHWF3.
    ...
    - pose proof IHWF as IHWF'.
      ...
      assert (HPD'1: D1 ++ [t] ≡[P] D1 ++ [t]) by reflexivity.
      specialize (IHWF3 HPD'1 HPD2 HPG1 HPG2 HG1_1 HG2) as (k1 & IHWF3).
      specialize (IHWF' _ HM2 _ _ b eq_refl IHu) as (IHWF'1 & IHWF'2 & IHWF'3). specialize (IHWF'3 HC); unfold_admissibilityH IHWF'3.
    ...
    - assert (HM1: n + m < S n + m) by lia.
      ...
      assert (HPD'1 : D1 ≡[P] D1) by reflexivity.
      assert (HPG'1 : G2 ++ [t] ≡[P] G2 ++ [t]) by reflexivity.
      assert (HPG'2 : G ++ [t] ≡[P] (G2 ++ [t]) ++ [shift_typ 0 b ([!]u)]) by (rewrite HPG1; apply Permutation_rel_assoc_swap).
      assert (HG'1 : m ⊣ c, (G2 ++ [t]), D2 ⊢pf).
      {eapply pfn_weakening. reflexivity. pfn_wf_ctx_solver. assumption. }
      specialize (IHWF3 HPD'1 HPD2 HPG'2 HPG'1 HG1 HG'1) as (k & IHWF3).
      exists (S k). eapply pfn_bang. 2: {eassumption. } assumption.
    ...
    - assert (HM1: n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF3 HC); unfold_admissibilityH IHWF3.
      specialize (IHWF3 c G G2 (D1 ++ [typ_subst 0 u0 t]) D2); listSimpl.
      assert (HPD'1: D1 ++ [typ_subst 0 u0 t] ≡[P] D1 ++[typ_subst 0 u0 t]) by reflexivity.
      specialize (IHWF3 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF3).
      exists (S k). eapply pfn_forall. eassumption. apply IHWF3. assumption.
    - assert (HM1 : n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ (1 + b) eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF3 HC); unfold_admissibilityH IHWF3.
      specialize (IHWF3 (1 + c) (shift_ctx 0 1 G) (shift_ctx 0 1 G2) (shift_ctx 0 1 D1 ++ [u0]) (shift_ctx 0 1 D2)); listSimpl.
      assert (HSD1: shift_ctx 0 1 D1 ++ [u0] ≡[P] shift_ctx 0 1 D1 ++ [u0]) by reflexivity.
      assert (HSD2: shift_ctx 0 1 D2 ≡[P] shift_ctx 0 (1 + b) [(dual ([!]u))]).
      {rewrite HPD2. simpl; repeat rewrite shift_typ_plus. reflexivity. }
      assert (HSG1 : shift_ctx 0 1 G ≡[P] shift_ctx 0 1 G2 ++ shift_ctx 0 (1 + b) [[!]u]).
      {rewrite HPG1, shift_ctx_app. simpl. repeat rewrite shift_typ_plus. reflexivity. }
      assert (HSG2 : shift_ctx 0 1 G2 ≡[P] shift_ctx 0 1 G2) by reflexivity.
      eapply (pfn_shift_inv 0 1) in HG2; try lia.
      replace (c + 1) with (1 + c) in * by lia.
  ...
  (*                   ∧ ((∀ n' m' b1 : nat, cut_admissibility_norm n' m' u b1) → cut_admissibility_bang n0 m0 u b0) *)
  (*                     ∧ ((∀ n' m' b1 : nat, cut_admissibility_norm n' m' u b1) → cut_admissibility_ques n0 m0 u b0)) *)
  (*                → cut_admissibility_norm n m ([forall] u) b *)
  (*                  ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' ([forall] u) b0) → cut_admissibility_bang n m ([forall] u) b) *)
  (*                    ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' ([forall] u) b0) → cut_admissibility_ques n m ([forall] u) b) *)
  (* IHY : ∀ y : nat, y < S n + S n0 → Acc lt y *)
  (* IHu : ∀ n m b : nat, *)
  (*         cut_admissibility_norm n m u b *)
  (*         ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u b0) → cut_admissibility_bang n m u b) *)
  (*           ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u b0) → cut_admissibility_ques n m u b) *)
  (* D1', D2', D : list typ *)
  (* HPD1 : D ≡[ P] D1' ++ shift_ctx 0 b [[forall] u] *)
  (* D2 : list typ *)
  (* HPD2 : D2 ≡[ P] D2' ++ shift_ctx 0 b [dual ([forall] u)] *)
  (* G : list typ *)
  (* HPG1, HPG2 : G ≡[ P] G ++ shift_ctx 0 b [] *)
  (* D1 : list typ *)
  (* u0 : typ *)
  (* c : nat *)
  (* H : c ⊢ u0 wf *)
  (* HG1 : n ⊣ c, G, D1 ++ [typ_subst 0 u0 (shift_typ 1 b u)] ⊢pf *)
  (* HPE2 : D1' ≡[ P] D1 *)
  (* D0 : list typ *)
  (* HPF1 : shift_typ 0 b (dual ([forall] u)) = [exists] shift_typ 1 b (dual u) *)
  (* HG2 : n0 ⊣ 1 + c, shift_ctx 0 1 G, shift_ctx 0 1 D0 ++ [shift_typ 1 b (dual u)] ⊢pf *)
  (* HPF2 : D2' ≡[ P] D0 *)
  (* ============================ *)
  (* ∃ k : nat, k ⊣ c, G, D1' ++ D2' ⊢pf *)

  Definition ctx_subst c u := List.map (typ_subst c u).

  
  Lemma ctx_subst_app :
    forall c G1 G2 u,
      ctx_subst c u (G1 ++ G2) = (ctx_subst c u G1) ++ (ctx_subst c u G2).
  Proof.
    intros b c G1 G2.
    unfold ctx_subst.
    rewrite map_app.
  ...
  Lemma typ_subst_shift0 : forall a c u t,
      a > 0 ->
      typ_subst c u (shift_typ c a t) = shift_typ c (a - 1) t.
  Proof.
    intros a c u t. revert a c u.
    induction t; intros; simpl; subst; auto.
    - destruct (Nat.ltb_spec x c); simpl.
      + apply Nat.ltb_lt in H0. rewrite H0; auto.
      + assert (not (a + x < c)) by lia. apply Nat.ltb_nlt in H1. rewrite H1; auto.
        assert (a + x <> c) by lia. apply Nat.eqb_neq in H2. rewrite H2. replace (Init.Nat.pred (a + x)) with (a - 1 + x) by lia. reflexivity.
    - rewrite IHt1, IHt2; auto.
    - rewrite IHt1, IHt2; auto.
    - rewrite IHt; auto.
    - rewrite IHt; auto.
  ...
  (* Lemma pfn_typ_weaken : forall n a c G D, *)
  (*     wf_ctx c G -> wf_ctx c D -> *)
  (*     n ⊣ (a + c), G, D ⊢pf -> *)
  (*                     n ⊣ c, G, D ⊢pf. *)
  (* Proof. *)
  (*   intros n a c G D HWFG HWFD HG. remember (a + c) as c'. *)
  (*   revert a c HWFG HWFD Heqc'. *)
  (*   induction HG; intros. *)
  (*   - constructor; pfn_wf_ctx_solver. *)
  (*   - eapply pfn_absorb; try eassumption. *)
  (*     + apply (wf_ctx_perm_iff H0) in HWFG. eapply HWFG. *)
  (*     + eapply IHHG; try eassumption. apply (wf_ctx_perm_iff H0) in HWFG. pfn_wf_ctx_solver. *)
  (*   - constructor; assumption. *)
  (*   - eapply pfn_one; try eassumption. *)
  (*     eapply IHHG; try eassumption. apply (wf_ctx_perm_iff H) in HWFD. pfn_wf_ctx_solver. *)
  (*   - eapply pfn_tensor; try eassumption. *)
  (*     eapply IHHG; try eassumption. *)

  Lemma pfn_subst : forall n c G G' D D' u t,
      c ⊢ u wf ->
      n ⊣ (1 + c), G, D ⊢pf ->
                      G ≡[P] shift_ctx 0 1 G' ->
                      D ≡[P] shift_ctx 0 1 D' ++ [t] ->
                      n ⊣ c, G', D' ++ [typ_subst 0 u t] ⊢pf.
  Proof.
    intros n c G G' D D' u t HWF HG HPG HPD.
    remember (1 + c) as c'.
    revert c G' D' t HWF HPG HPD Heqc'.
    induction HG; intros.
    - eapply Permutation_rel_split2 in HPD.
      destruct HPD as [[E1 [HPE1 HPE2]] | [E2 [HPE1 HPE2]]].
      + symmetry in HPE1. apply Permutation_rel_singleton_nil in HPE1 as (-> & ->); simpl in *.
        
        
  

  Lemma cut_admissibility_forall_norm:
    forall u x n m b
      (IHY : ∀ y : nat, y < x → Acc lt y)
      (IHWF : ∀ y : nat,
          y < x
  ...
  Proof.
    ...
    - subst. assert (HM1: n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G' (D ++ [t]) (D1' ++ [t]) D2 D2').
      assert (HPD'1: D ++ [t] ≡[P] (D1' ++ [t]) ++ [shift_typ 0 b ([forall] u)]). {rewrite HPD1. apply Permutation_rel_assoc_swap. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2).
      destruct IHWF1 as (k & IHWF1).
      exists (S k). eapply pfn_absorb. eassumption. eassumption.
      eapply pfn_perm_rel. reflexivity. eapply Permutation_rel_assoc_swap. assumption.
    ...
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H.
      ...
      specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G (D' ++ [t] ++ [u0]) (E1 ++ [t] ++ [u0]) D2 D2').
      assert (HPD'1: D' ++ [t] ++ [u0] ≡[P] (E1 ++ [t] ++ [u0] ) ++ [shift_typ 0 b ([forall] u)]). {rewrite HPE2, <- HPE3. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
      exists (S k). eapply pfn_tensor. 2: {rewrite HPE1. apply Permutation_rel_assoc_swap. }
      eapply pfn_perm_rel. reflexivity. assert ((E1 ++ [t] ++ [u0]) ++ D2' ≡[P] ((E1 ++ D2') ++ [t] ++ [u0])). {clear_permrel. convertTactics.convert_multisetperm. permutation_solver. } eassumption. assumption.
    - rewrite HPD1, app_assoc in H. apply Permutation_rel_split_last in H.
      ...
      + subst. assert (HM1: n1 + m < S (Nat.max n1 n2) + m) by lia.
        specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G (D1 ++ [t]) (F1 ++ [t]) D2 D2').
        assert (HPD'1: D1 ++ [t] ≡[P]  (F1 ++ [t]) ++ [shift_typ 0 b ([forall] u)]). {rewrite HPF1. apply Permutation_rel_assoc_swap. }
        specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1_1 HG2) as (k & IHWF1).
        exists (S (Nat.max k n2)). eapply pfn_par.
        * 
          eapply pfn_perm_rel. reflexivity. assert ((F1 ++ [t]) ++ D2' ≡[P] (F1 ++ D2') ++ [t]) by apply Permutation_rel_assoc_swap. apply H. assumption.
      ...
      + subst. assert (HM1: n2 + m < S (Nat.max n1 n2) + m) by lia.
        specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G (D0 ++ [u0]) (F2 ++ [u0]) D2 D2').
        assert (HPD'1: D0 ++ [u0] ≡[P] (F2 ++ [u0]) ++ [shift_typ 0 b ([forall] u)]). {rewrite HPF1. apply Permutation_rel_assoc_swap. }
        specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1_2 HG2) as (k & IHWF1).
        exists (S (Nat.max n1 k)). eapply pfn_par.
        * 
          eapply HG1_1.
    ...
    - symmetry in HPD1. apply Permutation_rel_singleton_nil in HPD1 as (_ & Hcontra). discriminate.
    - rewrite HPD1 in H0. apply Permutation_rel_split_last in H0. 
      destruct H0 as [[HPE1 HPE2] | [E1 [E2 [HPE1 [HPE2 HPE3]]]]].
      + injection HPE1. intros; subst. clear HPE1.
        destruct HG2.
        * eapply cut_admissibility_id2'. eassumption. rewrite dual_involutive. eassumption.
          eapply pfn_forall. apply H. eassumption. rewrite <- HPE2. assumption.
        * assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 c G' (D1 ++ [shift_typ 0 b ([forall]u)]) D1' (D0 ++ [t]) (D2' ++ [t])).
          assert (HPD'1: D1 ++ [shift_typ 0 b ([forall]u)] ≡[P] D1' ++ [shift_typ 0 b ([forall] u)]). {rewrite HPE2. reflexivity. }
          assert (HPD'2: D0 ++ [t] ≡[P] (D2' ++ [t]) ++ [shift_typ 0 b (dual ([forall] u))]). {rewrite HPD2. apply Permutation_rel_assoc_swap. }
          eapply pfn_forall in HG1. 3: {reflexivity. } 2: {assumption. }
          specialize (IHWF1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
          exists (S k). eapply pfn_absorb. eassumption. eassumption. rewrite <- app_assoc. assumption. 
        * symmetry in HPD2. apply Permutation_rel_singleton_nil in HPD2 as (_ & Hcontra). discriminate.
        * rewrite HPD2 in H0. apply Permutation_rel_split_last in H0.
          ...
          assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 c G (D1 ++ [shift_typ 0 b ([forall]u)]) D1' D' F2).
          assert (HPD'1: D1 ++ [shift_typ 0 b ([forall]u)] ≡[P] D1' ++ [shift_typ 0 b ([forall] u)]). {rewrite HPE2. reflexivity. }
          eapply pfn_forall in HG1. 3: {reflexivity. } 2: {assumption. }
          specialize (IHWF1 HPD'1 HPF2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
          exists (S k). eapply pfn_one. 2: {rewrite HPF1, HPF3, app_assoc. reflexivity. } assumption.
        * rewrite HPD2 in H0. apply Permutation_rel_split_last in H0.
          ...
          assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 c G (D1 ++ [shift_typ 0 b ([forall]u)]) D1' (D' ++ [t] ++ [u1]) (F1 ++ [t] ++ [u1])).
          assert (HPD'1: D1 ++ [shift_typ 0 b ([forall]u)] ≡[P] D1' ++ [shift_typ 0 b ([forall]u)]). {rewrite HPE2. reflexivity. }
          assert (HPD'2: D' ++ [t] ++ [u1] ≡[P] (F1 ++ [t] ++ [u1]) ++ [shift_typ 0 b (dual ([forall]u))]). {rewrite HPF2, <- HPF3. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. }
          eapply pfn_forall in HG1. 3: {reflexivity. } 2: {assumption. }
          specialize (IHWF1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
          exists (S k). eapply pfn_tensor.
          -- rewrite app_assoc in IHWF1. apply IHWF1.
          -- rewrite HPF1. rewrite app_assoc. reflexivity.
        * rewrite HPD2, app_assoc in H0. apply Permutation_rel_split_last in H0.
          destruct H0 as [[HPF1 HPF2] | [F1 [F2 [HPF1 [HPF2 HPF3]]]]]; try discriminate.
          apply Permutation_rel_split2 in HPF2.
          destruct HPF2 as [[I1 [HPI1 HPI2]] | [I2 [HPI1 HPI2]]].
          -- assert (HM1: S n + n1 < S n + S (Nat.max n1 n2)) by lia.
             specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
             eapply pfn_forall in HG1. 3: {reflexivity. } 2: {assumption. }
             specialize (IHWF1 c G (D1 ++ [shift_typ 0 b ([forall]u)]) D1' (D0 ++ [t]) (I1 ++ [t])).
             assert (HPD'1: D1 ++ [shift_typ 0 b ([forall]u)] ≡[P] D1' ++ [shift_typ 0 b ([forall]u)]). {rewrite HPE2. reflexivity. }
             assert (HPD'2: D0 ++ [t] ≡[P] (I1 ++ [t]) ++ [shift_typ 0 b (dual ([forall]u))]). {rewrite HPI1. apply Permutation_rel_assoc_swap. }
             specialize (IHWF1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2_1) as (k & IHWF1).
             exists (S (Nat.max k n2)). eapply pfn_par.
             3: {assert (D1' ++ D2' ≡[P] (D1' ++ I1) ++ D2 ++ [t_par t u1]). {rewrite HPF1 , HPF3, <- HPI2. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. } eassumption. }
             ++ rewrite <- app_assoc. assumption.
             ++ assumption.
          -- assert (HM1: S n + n2 < S n + S (Nat.max n1 n2)) by lia.
             specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
             specialize (IHWF1 c G (D1 ++ [shift_typ 0 b ([forall]u)]) D1' (D2 ++ [u1]) (I2 ++ [u1])).
             assert (HPD'1: D1 ++ [shift_typ 0 b ([forall] u)] ≡[P] D1' ++ [shift_typ 0 b ([forall]u)]). {rewrite HPE2. reflexivity. }
             assert (HPD'2: D2 ++ [u1] ≡[P] (I2 ++ [u1]) ++ [shift_typ 0 b (dual ([forall]u))]). {rewrite HPI1. apply Permutation_rel_assoc_swap. }
             eapply pfn_forall in HG1. 3: {reflexivity. } 2: {assumption. }
             specialize (IHWF1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2_2) as (k & IHWF1).
             exists (S (Nat.max n1 k)). eapply pfn_par.
             3: {assert (D1' ++ D2' ≡[P] D0 ++ (D1' ++ I2) ++ [t_par t u1]). {rewrite HPF1, HPF3, <- HPI2. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. } eassumption. }
             ++ assumption.
             ++ rewrite <- app_assoc. apply IHWF1.
        * rewrite HPD2 in H0. apply Permutation_rel_split_last in H0.
          destruct H0 as [[HPF1 HPF2] | [F1 [F2 [HPF1 [HPF2 HPF3]]]]]; try discriminate.
          assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 c (G ++ [t]) (D1 ++ [shift_typ 0 b ([forall]u)]) D1' D0 F2); listSimpl.
          assert (HPD'1: D1 ++ [shift_typ 0 b ([forall]u)] ≡[P] D1' ++ [shift_typ 0 b ([forall]u)]). {rewrite HPE2. reflexivity. }
          eapply pfn_forall in HG1. 3: {reflexivity. } 2: {assumption. }
          assert (HG'1: S n ⊣ c, G ++ [t], D1 ++ [shift_typ 0 b ([forall]u)] ⊢pf). {eapply pfn_weakening. reflexivity. pfn_wf_ctx_solver. assumption. }
          assert (HPG'1: G ++ [t] ≡[P] G ++ [t]) by reflexivity.
          specialize (IHWF1 HPD'1 HPF2 HPG'1 HPG'1 HG'1 HG2) as (k & IHWF1).
          exists (S k). eapply pfn_bang. 2: {rewrite HPF1, HPF3, app_assoc. reflexivity. } assumption.
        * symmetry in HPD2. apply Permutation_rel_singleton_nil in HPD2 as (_ & Hcontra). discriminate.
        * rewrite HPD2 in H1. apply Permutation_rel_split_last in H1.
          destruct H1 as [[HPF1 HPF2] |[F1 [F2 [HPF1 [HPF2 HPF3]]]]]; try discriminate.
          assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ b eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 c G D D1' (D0 ++ [typ_subst 0 u1 t]) (F2 ++ [typ_subst 0 u1 t])); listSimpl.
          assert (HPD'2: D0 ++ [typ_subst 0 u1 t] ≡[P] (F2 ++ [typ_subst 0 u1 t]) ++ [shift_typ 0 b (dual ([forall]u))]). {rewrite HPF2. apply Permutation_rel_assoc_swap. }
          eapply pfn_forall in HG1. 3: {rewrite <- HPE2. apply HPD1. } 2: {assumption. }
          specialize (IHWF1 HPD1 HPD'2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
          exists (S k). eapply pfn_forall. eassumption. rewrite app_assoc in IHWF1. eassumption. rewrite HPF1, HPF3, app_assoc. reflexivity.
        * rewrite HPD2 in H0. apply Permutation_rel_split_last in H0.
          destruct H0 as [[HPF1 HPF2] |[F1 [F2 [HPF1 [HPF2 HPF3]]]]].
          -- 
            injection HPF1. intros <-.
            eapply pfn_forall in HG1. 2: {assumption. } 2: {rewrite <- HPE2, <- HPD1. reflexivity. }
            assert (HM1: S n + n0 < S n + S n0) by lia.
            specialize (IHWF _ HM1 _ _ (b + 1) eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
            
            specialize (IHWF)
            eapply pfn_exists in HG2. 2: {rewrite <- HPF2. eapply HPD2. }
            
            specialize (IHu (S n) (S n0) (1 + b)) as (IHu1 & _ & _); unfold_admissibilityH IHu1.
            


            
            specialize (IHu1 (S n) n0 b) as (IHu1_1 & _ & _); unfold_admissibilityH IHu1_1.
            specialize (IHu1_1 c G (D' ++ [shift_typ 0 b u1] ++ [shift_typ 0 b u2]) (D' ++ [shift_typ 0 b u2]) (D1 ++ [shift_typ 0 b (dual u1)]) D1); listSimpl.
            assert (HPD'1: D' ++ [shift_typ 0 b u1] ++ [shift_typ 0 b u2] ≡[P] (D' ++ [shift_typ 0 b u2]) ++ [shift_typ 0 b u1]). {rewrite app_assoc. apply Permutation_rel_assoc_swap. }
            assert (HPD'2: D1 ++ [shift_typ 0 b (dual u1)] ≡[P] D1 ++ [shift_typ 0 b (dual u1)]) by reflexivity.
            specialize (IHu1_1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2_1) as (k1 & IHu1_1).
          ...
          -- assert (HM1: S n + n2 < S n + S (Nat.max n1 n2)) by lia.
             ...
             ++ assumption.
             ++ rewrite <- app_assoc. apply IHWF1.
          -- injection HPF1; intros; subst. clear HPF1.
      ...
      + assert (HM1 : n + m < S n + m) by lia.
        ...
        assert (HPD'1: D1 ++ [typ_subst c u0 t] ≡[P] (E2 ++ [typ_subst c u0 t]) ++ [[forall]u]). {rewrite HPE2. apply Permutation_rel_assoc_swap. }
        specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
        exists (S k). eapply pfn_forall. eassumption. 2: {rewrite HPE1, HPE3. apply Permutation_rel_assoc_swap. }
        eapply pfn_perm_rel. reflexivity. apply Permutation_rel_assoc_swap. assumption.
    - admit.
  ...
  Lemma cut_admissibility_forall :
    forall u n m b
      (IHu : ∀ n m b : nat,
          cut_admissibility_norm n m u b
          ∧ ((∀ n' m' b0 : nat, cut_admissibility_norm n' m' u b0) → cut_admissibility_bang n m u b)
  ...
  Proof.
    ...
    induction (lt_wf o); intros.
    repeat split.
    - intros. eapply cut_admissibility_forall_norm; eassumption.
    - intros. eapply cut_admissibility_forall_bang; eassumption.
    - intros. eapply cut_admissibility_forall_ques; eassumption.
  Qed.
  ...
  Proof. 
    ...
    - apply (cut_admissibility_dual (t_par u1 u2)).
      ...
      specialize (proj1 (cut_admissibility_dual _) IHu1) as IHu1'. clear IHu1.
      specialize (proj1 (cut_admissibility_dual _) IHu2) as IHu2'. clear IHu2.
      pose proof (cut_admissibility_tensor (dual u1) (dual u2) n m b IHu1' IHu2').
      auto.
    - intros. eapply cut_admissibility_bang_case; eassumption.
    - apply (cut_admissibility_dual ([?] u)).
      intros. 
      ...
      (IHWF : ∀ y : nat,
          ...
          → ∀ n m : nat,
             ...
             → (∀ n0 m0 : nat,
               ...
               → cut_admissibility_norm n m ([forall] u)
                 ∧ ((∀ n' m' : nat, cut_admissibility_norm n' m' ([forall] u))
                    → cut_admissibility_bang n m ([forall] u))
                   ∧ ((∀ n' m' : nat, cut_admissibility_norm n' m' ([forall] u))
                      → cut_admissibility_ques n m ([forall] u)))
      (Heqo : x = n + m)
      (IHu : ∀ n m : nat,
  ...
  Proof.
    ...
    - subst. assert (HM1: n + m < S n + m) by lia.
      ...
      specialize (IHWF1 c G' (D ++ [t]) (D1' ++ [t]) D2 D2').
      assert (HPD'1: D ++ [t] ≡[P] (D1' ++ [t]) ++ [[forall] u]). {rewrite HPD1. apply Permutation_rel_assoc_swap. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2).
      destruct IHWF1 as (k & IHWF1).
      exists (S k). eapply pfn_absorb. eassumption. eassumption.
      eapply pfn_perm_rel. reflexivity. eapply Permutation_rel_assoc_swap. assumption.
    ...
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H.
      ...
      specialize (IHWF _ HM1 _ _ eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G (D' ++ [t] ++ [u0]) (E1 ++ [t] ++ [u0]) D2 D2').
      assert (HPD'1: D' ++ [t] ++ [u0] ≡[P] (E1 ++ [t] ++ [u0] ) ++ [[forall] u]). {rewrite HPE2, <- HPE3. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
      exists (S k). eapply pfn_tensor. 2: {rewrite HPE1. apply Permutation_rel_assoc_swap. }
      eapply pfn_perm_rel. reflexivity. assert ((E1 ++ [t] ++ [u0]) ++ D2' ≡[P] ((E1 ++ D2') ++ [t] ++ [u0])). {clear_permrel. convertTactics.convert_multisetperm. permutation_solver. } eassumption. assumption.
    - rewrite HPD1, app_assoc in H. apply Permutation_rel_split_last in H.
      ...
      + subst. assert (HM1: n1 + m < S (Nat.max n1 n2) + m) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G (D1 ++ [t]) (F1 ++ [t]) D2 D2').
        assert (HPD'1: D1 ++ [t] ≡[P]  (F1 ++ [t]) ++ [[forall] u]). {rewrite HPF1. apply Permutation_rel_assoc_swap. }
        specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1_1 HG2) as (k & IHWF1).
        exists (S (Nat.max k n2)). eapply pfn_par.
        * 
          eapply pfn_perm_rel. reflexivity. assert ((F1 ++ [t]) ++ D2' ≡[P] (F1 ++ D2') ++ [t]) by apply Permutation_rel_assoc_swap. apply H. assumption.
      ...
      + subst. assert (HM1: n2 + m < S (Nat.max n1 n2) + m) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G (D0 ++ [u0]) (F2 ++ [u0]) D2 D2').
        assert (HPD'1: D0 ++ [u0] ≡[P] (F2 ++ [u0]) ++ [[forall] u]). {rewrite HPF1. apply Permutation_rel_assoc_swap. }
        specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1_2 HG2) as (k & IHWF1).
        exists (S (Nat.max n1 k)). eapply pfn_par.
        * 
          eapply HG1_1.
    ...
    - rewrite HPD1 in H0. apply Permutation_rel_split_last in H0. 
      ...
      + injection HPE1. intros; subst. clear HPE1.
        ...
        * assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 c G' (D1 ++ [[forall]t]) D1' (D0 ++ [t0]) (D2' ++ [t0])).
          assert (HPD'1: D1 ++ [[forall]t] ≡[P] D1' ++ [[forall] t]). {rewrite HPE2. reflexivity. }
          assert (HPD'2: D0 ++ [t0] ≡[P] (D2' ++ [t0]) ++ [dual ([forall] t)]). {rewrite HPD2. apply Permutation_rel_assoc_swap. }
          eapply pfn_forall in HG1. 3: {reflexivity. } 2: {assumption. }
          specialize (IHWF1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
          exists (S k). eapply pfn_absorb. eassumption. eassumption. rewrite <- app_assoc. assumption. 
        * symmetry in HPD2. apply Permutation_rel_singleton_nil in HPD2 as (_ & Hcontra). discriminate.
        * rewrite HPD2 in H0. apply Permutation_rel_split_last in H0.
          ...
          assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 c G (D1 ++ [[forall]t]) D1' D' F2).
          assert (HPD'1: D1 ++ [[forall]t] ≡[P] D1' ++ [[forall] t]). {rewrite HPE2. reflexivity. }
          eapply pfn_forall in HG1. 3: {reflexivity. } 2: {assumption. }
          specialize (IHWF1 HPD'1 HPF2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
          exists (S k). eapply pfn_one. 2: {rewrite HPF1, HPF3, app_assoc. reflexivity. } assumption.
        * rewrite HPD2 in H0. apply Permutation_rel_split_last in H0.
          ...
          assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 c G (D1 ++ [[forall]t]) D1' (D' ++ [t0] ++ [u]) (F1 ++ [t0] ++ [u])).
          assert (HPD'1: D1 ++ [[forall]t] ≡[P] D1' ++ [[forall]t]). {rewrite HPE2. reflexivity. }
          assert (HPD'2: D' ++ [t0] ++ [u] ≡[P] (F1 ++ [t0] ++ [u]) ++ [dual ([forall]t)]). {rewrite HPF2, <- HPF3. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. }
          eapply pfn_forall in HG1. 3: {reflexivity. } 2: {assumption. }
          specialize (IHWF1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
          exists (S k). eapply pfn_tensor.
          -- rewrite app_assoc in IHWF1. apply IHWF1.
        ...
        * rewrite HPD2, app_assoc in H0. apply Permutation_rel_split_last in H0.
          ...
          -- assert (HM1: S n + n1 < S n + S (Nat.max n1 n2)) by lia.
             specialize (IHWF _ HM1 _ _ eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
             eapply pfn_forall in HG1. 3: {reflexivity. } 2: {assumption. }
             specialize (IHWF1 c G (D1 ++ [[forall]t]) D1' (D0 ++ [t0]) (I1 ++ [t0])).
             assert (HPD'1: D1 ++ [[forall]t] ≡[P] D1' ++ [[forall]t]). {rewrite HPE2. reflexivity. }
             assert (HPD'2: D0 ++ [t0] ≡[P] (I1 ++ [t0]) ++ [dual ([forall]t)]). {rewrite HPI1. apply Permutation_rel_assoc_swap. }
             specialize (IHWF1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2_1) as (k & IHWF1).
             exists (S (Nat.max k n2)). eapply pfn_par.
          ...
          -- assert (HM1: S n + n2 < S n + S (Nat.max n1 n2)) by lia.
             specialize (IHWF _ HM1 _ _ eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
             specialize (IHWF1 c G (D1 ++ [[forall]t]) D1' (D2 ++ [u]) (I2 ++ [u])).
             assert (HPD'1: D1 ++ [[forall] t] ≡[P] D1' ++ [[forall]t]). {rewrite HPE2. reflexivity. }
             assert (HPD'2: D2 ++ [u] ≡[P] (I2 ++ [u]) ++ [dual ([forall]t)]). {rewrite HPI1. apply Permutation_rel_assoc_swap. }
             eapply pfn_forall in HG1. 3: {reflexivity. } 2: {assumption. }
             specialize (IHWF1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2_2) as (k & IHWF1).
             exists (S (Nat.max n1 k)). eapply pfn_par.
        ...
        * rewrite HPD2 in H0. apply Permutation_rel_split_last in H0.
          ...
          assert (HM1: S n + n0 < S n + S n0) by lia.
          specialize (IHWF _ HM1 _ _ eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
          specialize (IHWF1 c (G ++ [t0]) (D1 ++ [[forall]t]) D1' D0 F2); listSimpl.
          assert (HPD'1: D1 ++ [[forall]t] ≡[P] D1' ++ [[forall]t]). {rewrite HPE2. reflexivity. }
          eapply pfn_forall in HG1. 3: {reflexivity. } 2: {assumption. }
          assert (HG'1: S n ⊣ c, G ++ [t0], D1 ++ [[forall]t] ⊢pf). {eapply pfn_weakening. reflexivity. pfn_wf_ctx_solver. assumption. }
        ...
        * rewrite HPD2 in H1. apply Permutation_rel_split_last in H1.
          ...
          assert (HPD'2: D0 ++ [typ_subst c u t0] ≡[P] (F2 ++ [typ_subst c u t0]) ++ [dual ([forall]t)]). {rewrite HPF2. apply Permutation_rel_assoc_swap. }
          eapply pfn_forall in HG1. 3: {rewrite <- HPE2. apply HPD1. } 2: {assumption. }
          specialize (IHWF1 HPD1 HPD'2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
          exists (S k). eapply pfn_forall. eassumption. rewrite app_assoc in IHWF1. eassumption. rewrite HPF1, HPF3, app_assoc. reflexivity.
        * admit.
  ...
  Lemma cut_admissibility_forall_bang:
    forall u x n m
      (IHY : ∀ y : nat, y < x → Acc lt y)
      (IHWF : ∀ y : nat,
          y < x
      ...
      (IHu : ∀ n m : nat,
          ...
          ∧ ((∀ n' m' : nat, cut_admissibility_norm n' m' u) → cut_admissibility_bang n m u)
          ∧ ((∀ n' m' : nat, cut_admissibility_norm n' m' u) → cut_admissibility_ques n m u))
      (HC: forall n m, cut_admissibility_norm n m ([forall] u)),
  cut_admissibility_bang n m ([forall] u).
  Proof.
    intros. unfold cut_admissibility_bang, cut_admissibility_t.
    intros c G1 G2 D1 D2 HPD1 HPD2 HPG1 HPG2 HG1 HG2. listSimpl. 
    ...
    - rewrite HPG2 in H0. apply Permutation_rel_split_last in H0.
      ...
      + 
        ...
        specialize (IHWF2 c G1 G' D1 (D ++ [dual ([forall] u)])); listSimpl.
        assert (HPD'1: D ++ [dual ([forall] u)] ≡[P] D ++ [dual ([forall]u)]) by reflexivity.
        specialize (IHWF2 HPD1 HPD'1 HPG1 HPG2 HG1 HG2) as (k & IHWF2).
        unfold_admissibilityH HC. specialize (HC n k c G1 D1 [] (D ++ [dual ([forall]u)]) D); listSimpl.
        specialize (HC HPD1 HPD'1 HPG1 HPG1 HG1 IHWF2) as (k2 & HC).
        eexists; eauto.
      + assert (HM1 : n + n0 < n + S n0) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF2 HC); unfold_admissibilityH IHWF2.
    ...
    - assert (HM1: n + n0 < n + S n0) by lia.
      ...
      assert (HPD'1 : D0 ≡[P] D0) by reflexivity.
      assert (HPG'1 : G1 ++ [t] ≡[P] G1 ++ [t]) by reflexivity.
      assert (HPG'2 : G ++ [t] ≡[P] (G1 ++ [t]) ++ [dual ([forall]u)]) by (rewrite HPG2; apply Permutation_rel_assoc_swap).
      assert (HG'1 : n ⊣ c, (G1 ++ [t]), D1 ⊢pf).
      {eapply pfn_weakening. reflexivity. pfn_wf_ctx_solver. assumption. }
      specialize (IHWF2 HPD1 HPD'1 HPG'1 HPG'2 HG'1 HG2) as (k & IHWF2).
      exists (S k). eapply pfn_bang. 2: {eassumption. } assumption.
  ...
  Lemma cut_admissibility_forall_ques:
    forall u x n m
      (IHY : ∀ y : nat, y < x → Acc lt y)
      (IHWF : ∀ y : nat,
          y < x
  ...
  Proof.
    ...
    - rewrite HPG1 in H0. apply Permutation_rel_split_last in H0.
      ...
      + 
        ...
        specialize (IHWF3 c G' G2 (D ++ [[forall] u]) D2); listSimpl.
        assert (HPD'1: D ++ [[forall]u] ≡[P] D ++ [[forall]u]) by reflexivity.
        specialize (IHWF3 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF3).
        unfold_admissibilityH HC. specialize (HC k m c G2 (D ++ [[forall]u]) D D2 []); listSimpl.
        specialize (HC HPD'1 HPD2 HPG2 HPG2 IHWF3 HG2) as (k2 & HC).
        eexists; eauto.
      + assert (HM1 : n + m < S n + m) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl IHu) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF3 HC); unfold_admissibilityH IHWF3.
    ...
    - assert (HM1: n + m < S n + m) by lia.
      ...
      assert (HPD'1 : D1 ≡[P] D1) by reflexivity.
      assert (HPG'1 : G2 ++ [t] ≡[P] G2 ++ [t]) by reflexivity.
      assert (HPG'2 : G ++ [t] ≡[P] (G2 ++ [t]) ++ [[forall]u]) by (rewrite HPG1; apply Permutation_rel_assoc_swap).
      assert (HG'1 : m ⊣ c, (G2 ++ [t]), D2 ⊢pf).
      {eapply pfn_weakening. reflexivity. pfn_wf_ctx_solver. assumption. }
      specialize (IHWF3 HPD'1 HPD2 HPG'2 HPG'1 HG1 HG'1) as (k & IHWF3).
      exists (S k). eapply pfn_bang. 2: {eassumption. } assumption.
  ...
  Lemma cut_admissibility_var_norm:
    forall p x z n m
      (IHY : ∀ y : nat, y < z → Acc lt y)
      (IHWF : ∀ y : nat,
          y < z
          → ∀ n m : nat,
             y = n + m
             → cut_admissibility_norm n m (t_var p x)
               ∧ ((∀ n' m' : nat, cut_admissibility_norm n' m' (t_var p x))
                  → cut_admissibility_bang n m (t_var p x))
                 ∧ ((∀ n' m' : nat, cut_admissibility_norm n' m' (t_var p x))
                    → cut_admissibility_ques n m (t_var p x)))
      (Heqo : z = n + m),
  cut_admissibility_norm n m (t_var p x).
  Proof.
    intros. unfold cut_admissibility_norm, cut_admissibility_t.
    ...
    - eapply cut_admissibility'_aux_id1; eassumption.
    - subst. assert (HM1: n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G' (D ++ [t]) (D1' ++ [t]) D2 D2').
      assert (HPD'1: D ++ [t] ≡[P] (D1' ++ [t]) ++ [t_var p x]). {rewrite HPD1. apply Permutation_rel_assoc_swap. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2).
      destruct IHWF1 as (k & IHWF1).
      exists (S k). eapply pfn_absorb. eassumption. eassumption.
      eapply pfn_perm_rel. reflexivity. eapply Permutation_rel_assoc_swap. assumption.
    ...
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H.
      ...
      subst. assert (HM1: n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G (D' ++ [t] ++ [u]) (E1 ++ [t] ++ [u]) D2 D2').
      assert (HPD'1: D' ++ [t] ++ [u] ≡[P] (E1 ++ [t] ++ [u] ) ++ [t_var p x]). {rewrite HPE2, <- HPE3. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
      exists (S k). eapply pfn_tensor. 2: {rewrite HPE1. apply Permutation_rel_assoc_swap. }
      eapply pfn_perm_rel. reflexivity. assert ((E1 ++ [t] ++ [u]) ++ D2' ≡[P] ((E1 ++ D2') ++ [t] ++ [u])). {clear_permrel. convertTactics.convert_multisetperm. permutation_solver. } eassumption. assumption.
    - rewrite HPD1, app_assoc in H. apply Permutation_rel_split_last in H.
      destruct H as [[HPE1 HPE2] | [E1 [E2 [HPE1 [HPE2 HPE3]]]]]; try discriminate.
      apply Permutation_rel_split2 in HPE2.
      destruct HPE2 as [[F1 [HPF1 HPF2]] | [F2 [HPF1 HPF2]]].
      + subst. assert (HM1: n1 + m < S (Nat.max n1 n2) + m) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G (D1 ++ [t]) (F1 ++ [t]) D2 D2').
        assert (HPD'1: D1 ++ [t] ≡[P]  (F1 ++ [t]) ++ [t_var p x]). {rewrite HPF1. apply Permutation_rel_assoc_swap. }
        specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1_1 HG2) as (k & IHWF1).
        exists (S (Nat.max k n2)). eapply pfn_par.
        * 
          eapply pfn_perm_rel. reflexivity. assert ((F1 ++ [t]) ++ D2' ≡[P] (F1 ++ D2') ++ [t]) by apply Permutation_rel_assoc_swap. apply H. assumption.
      ...
      + subst. assert (HM1: n2 + m < S (Nat.max n1 n2) + m) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G (D0 ++ [u]) (F2 ++ [u]) D2 D2').
        assert (HPD'1: D0 ++ [u] ≡[P] (F2 ++ [u]) ++ [t_var p x]). {rewrite HPF1. apply Permutation_rel_assoc_swap. }
        specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1_2 HG2) as (k & IHWF1).
        exists (S (Nat.max n1 k)). eapply pfn_par.
        * 
          eapply HG1_1.
        * 
          eapply pfn_perm_rel. reflexivity. assert ((F2 ++ [u]) ++ D2' ≡[P] (F2 ++ D2') ++ [u]) by apply Permutation_rel_assoc_swap. apply H. assumption.
        * 
          rewrite HPE1, HPE3, <- HPF2. clear_permrel. convertTactics.convert_multisetperm. permutation_solver.
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H.
      destruct H as [[HPE1 HPE2] | [E1 [E2 [HPE1 [HPE2 HPE3]]]]]; try discriminate.
    ...
    - rewrite HPD1 in H0. apply Permutation_rel_split_last in H0. 
      destruct H0 as [[HPE1 HPE2] | [E1 [E2 [HPE1 [HPE2 HPE3]]]]]; try discriminate.
      assert (HM1 : n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IWHF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G (D1 ++ [typ_subst c u t]) (E2 ++ [typ_subst c u t]) D2 D2').
      assert (HPD'1: D1 ++ [typ_subst c u t] ≡[P] (E2 ++ [typ_subst c u t]) ++ [t_var p x]). {rewrite HPE2. apply Permutation_rel_assoc_swap. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
      exists (S k). eapply pfn_forall. eassumption. 2: {rewrite HPE1, HPE3. apply Permutation_rel_assoc_swap. }
      eapply pfn_perm_rel. reflexivity. apply Permutation_rel_assoc_swap. assumption.
    - admit.
  ...
  Lemma cut_admissibility_var_bang:
    forall p x z n m
      (IHY : ∀ y : nat, y < z → Acc lt y)
      (IHWF : ∀ y : nat,
          y < z
             ...
             → cut_admissibility_norm n m (t_var p x)
               ∧ ((∀ n' m' : nat,
                 ...
                 ∧ ((∀ n' m' : nat,
                    ...
                    → cut_admissibility_ques n m (t_var p x)))
      (Heqo : z = n + m)
      (HC: forall n m, cut_admissibility_norm n m (t_var p x)),
  cut_admissibility_bang n m (t_var p x).
  Proof.
    intros. unfold cut_admissibility_bang, cut_admissibility_t.
    intros c G1 G2 D1 D2 HPD1 HPD2 HPG1 HPG2 HG1 HG2. listSimpl. 
    ...
    - rewrite HPG2 in H0. apply Permutation_rel_split_last in H0.
      ...
      + 
        subst.
        assert (HM1: n + n0 < n + S n0) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). specialize (IHWF2 HC). unfold_admissibilityH IHWF2.
        specialize (IHWF2 c G1 G' D1 (D ++ [dual (t_var p x)])); listSimpl.
        assert (HPD'1: D ++ [dual (t_var p x)] ≡[P] D ++ [dual (t_var p x)]) by reflexivity.
        specialize (IHWF2 HPD1 HPD'1 HPG1 HPG2 HG1 HG2) as (k & IHWF2).
        unfold_admissibilityH HC. specialize (HC n k c G1 D1 [] (D ++ [dual (t_var p x)]) D); listSimpl.
        specialize (HC HPD1 HPD'1 HPG1 HPG1 HG1 IHWF2) as (k2 & HC).
        eexists; eauto.
      + assert (HM1 : n + n0 < n + S n0) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF2 HC); unfold_admissibilityH IHWF2.
    ...
    - pose proof IHWF as IHWF'.
      ...
      assert (HPD'1: D0 ++ [t] ≡[P] D0 ++ [t]) by reflexivity.
      specialize (IHWF2 HPD1 HPD'1 HPG1 HPG2 HG1 HG2_1) as (k1 & IHWF2).
      specialize (IHWF' _ HM2 _ _ eq_refl) as (IHWF'1 & IHWF'2 & IHWF'3). specialize (IHWF'2 HC); unfold_admissibilityH IHWF'2.
      specialize (IHWF'2 c G1 G D1 (D2 ++ [u])); listSimpl.
      assert (HPD'2 : D2 ++ [u] ≡[P] D2 ++ [u]) by reflexivity.
      specialize (IHWF'2 HPD1 HPD'2 HPG1 HPG2 HG1 HG2_2) as (k2 & IHWF'2).
      exists (S (Nat.max k1 k2)). eapply pfn_par; try eassumption.
    - assert (HM1: n + n0 < n + S n0) by lia.
      ...
      assert (HPD'1 : D0 ≡[P] D0) by reflexivity.
      assert (HPG'1 : G1 ++ [t] ≡[P] G1 ++ [t]) by reflexivity.
      assert (HPG'2 : G ++ [t] ≡[P] (G1 ++ [t]) ++ [dual (t_var p x)]) by (rewrite HPG2; apply Permutation_rel_assoc_swap).
      assert (HG'1 : n ⊣ c, (G1 ++ [t]), D1 ⊢pf).
      {eapply pfn_weakening. reflexivity. pfn_wf_ctx_solver. assumption. }
      specialize (IHWF2 HPD1 HPD'1 HPG'1 HPG'2 HG'1 HG2) as (k & IHWF2).
      exists (S k). eapply pfn_bang. 2: {eassumption. } assumption.
  ...
  Lemma cut_admissibility_var_ques:
    forall p x z n m
      (IHY : ∀ y : nat, y < z → Acc lt y)
      (IHWF : ∀ y : nat,
          y < z
  ...
  Proof.
    ...
    - rewrite HPG1 in H0. apply Permutation_rel_split_last in H0.
      ...
      + 
        subst.
        assert (HM1: n + m < S n + m) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). specialize (IHWF3 HC). unfold_admissibilityH IHWF3.
        specialize (IHWF3 c G' G2 (D ++ [t_var p x]) D2); listSimpl.
        assert (HPD'1: D ++ [t_var p x] ≡[P] D ++ [t_var p x]) by reflexivity.
        specialize (IHWF3 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF3).
        unfold_admissibilityH HC. specialize (HC k m c G2 (D ++ [t_var p x]) D D2 []); listSimpl.
        specialize (HC HPD'1 HPD2 HPG2 HPG2 IHWF3 HG2) as (k2 & HC).
        eexists; eauto.
      + assert (HM1 : n + m < S n + m) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF3 HC); unfold_admissibilityH IHWF3.
    ...
    - pose proof IHWF as IHWF'.
      ...
      assert (HPD'1: D1 ++ [t] ≡[P] D1 ++ [t]) by reflexivity.
      specialize (IHWF3 HPD'1 HPD2 HPG1 HPG2 HG1_1 HG2) as (k1 & IHWF3).
      specialize (IHWF' _ HM2 _ _ eq_refl) as (IHWF'1 & IHWF'2 & IHWF'3). specialize (IHWF'3 HC); unfold_admissibilityH IHWF'3.
      specialize (IHWF'3 c G G2 (D0 ++ [u]) D2); listSimpl.
      assert (HPD'2 : D0 ++ [u] ≡[P] D0 ++ [u]) by reflexivity.
      specialize (IHWF'3 HPD'2 HPD2 HPG1 HPG2 HG1_2 HG2) as (k2 & IHWF'3).
      exists (S (Nat.max k1 k2)). eapply pfn_par; try eassumption.
    - assert (HM1: n + m < S n + m) by lia.
      ...
      assert (HPD'1 : D1 ≡[P] D1) by reflexivity.
      assert (HPG'1 : G2 ++ [t] ≡[P] G2 ++ [t]) by reflexivity.
      assert (HPG'2 : G ++ [t] ≡[P] (G2 ++ [t]) ++ [t_var p x]) by (rewrite HPG1; apply Permutation_rel_assoc_swap).
      assert (HG'1 : m ⊣ c, (G2 ++ [t]), D2 ⊢pf).
      {eapply pfn_weakening. reflexivity. pfn_wf_ctx_solver. assumption. }
      specialize (IHWF3 HPD'1 HPD2 HPG'2 HPG'1 HG1 HG'1) as (k & IHWF3).
      exists (S k). eapply pfn_bang. 2: {eassumption. } assumption.
  ...
  Proof.
    ...
    induction (lt_wf o); intros.
    repeat split.
    - intros. eapply cut_admissibility_var_norm; eassumption.
    - intros. eapply cut_admissibility_var_bang; eassumption.
    - intros. eapply cut_admissibility_var_ques; eassumption.
  Qed.

  Lemma cut_admissibility_other_norm:
    forall p x z n m
      (IHY : ∀ y : nat, y < z → Acc lt y)
      (IHWF : ∀ y : nat,
          y < z
          → ∀ n m : nat,
             y = n + m
             → cut_admissibility_norm n m
                 (t_base p (b_other x))
               ∧ ((∀ n' m' : nat,
                     cut_admissibility_norm n' m'
                       (t_base p (b_other x)))
                  → cut_admissibility_bang n m
                      (t_base p (b_other x)))
                 ∧ ((∀ n' m' : nat,
                       cut_admissibility_norm n' m'
                         (t_base p (b_other x)))
                    → cut_admissibility_ques n m
                        (t_base p (b_other x))))
      (Heqo : z = n + m),
  cut_admissibility_norm n m (t_base p (b_other x)).
  Proof.
    intros. unfold cut_admissibility_norm, cut_admissibility_t.
    intros c G D1 D1' D2 D2' HPD1 HPD2 HPG1 HPG2 HG1 HG2.
    destruct HG1; subst.
    ...
    - subst. assert (HM1: n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G' (D ++ [t]) (D1' ++ [t]) D2 D2').
      assert (HPD'1: D ++ [t] ≡[P] (D1' ++ [t]) ++ [t_base p (b_other x)]). {rewrite HPD1. apply Permutation_rel_assoc_swap. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2).
      destruct IHWF1 as (k & IHWF1).
      exists (S k). eapply pfn_absorb. eassumption. eassumption.
      eapply pfn_perm_rel. reflexivity. eapply Permutation_rel_assoc_swap. assumption.
    ...
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H.
      ...
      specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G (D' ++ [t] ++ [u]) (E1 ++ [t] ++ [u]) D2 D2').
      assert (HPD'1: D' ++ [t] ++ [u] ≡[P] (E1 ++ [t] ++ [u] ) ++ [t_base p (b_other x)]). {rewrite HPE2, <- HPE3. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
      exists (S k). eapply pfn_tensor. 2: {rewrite HPE1. apply Permutation_rel_assoc_swap. }
      eapply pfn_perm_rel. reflexivity. assert ((E1 ++ [t] ++ [u]) ++ D2' ≡[P] ((E1 ++ D2') ++ [t] ++ [u])). {clear_permrel. convertTactics.convert_multisetperm. permutation_solver. } eassumption. assumption.
    - rewrite HPD1, app_assoc in H. apply Permutation_rel_split_last in H.
      ...
      + subst. assert (HM1: n1 + m < S (Nat.max n1 n2) + m) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G (D1 ++ [t]) (F1 ++ [t]) D2 D2').
        assert (HPD'1: D1 ++ [t] ≡[P]  (F1 ++ [t]) ++ [t_base p (b_other x)]). {rewrite HPF1. apply Permutation_rel_assoc_swap. }
        specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1_1 HG2) as (k & IHWF1).
        exists (S (Nat.max k n2)). eapply pfn_par.
        * 
          eapply pfn_perm_rel. reflexivity. assert ((F1 ++ [t]) ++ D2' ≡[P] (F1 ++ D2') ++ [t]) by apply Permutation_rel_assoc_swap. apply H. assumption.
      ...
      + subst. assert (HM1: n2 + m < S (Nat.max n1 n2) + m) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G (D0 ++ [u]) (F2 ++ [u]) D2 D2').
        assert (HPD'1: D0 ++ [u] ≡[P] (F2 ++ [u]) ++ [t_base p (b_other x)]). {rewrite HPF1. apply Permutation_rel_assoc_swap. }
        specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1_2 HG2) as (k & IHWF1).
        exists (S (Nat.max n1 k)). eapply pfn_par.
        * 
          eapply HG1_1.
    ...
    - rewrite HPD1 in H0. apply Permutation_rel_split_last in H0. 
      ...
      specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IWHF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G (D1 ++ [typ_subst c u t]) (E2 ++ [typ_subst c u t]) D2 D2').
      assert (HPD'1: D1 ++ [typ_subst c u t] ≡[P] (E2 ++ [typ_subst c u t]) ++ [t_base p (b_other x)]). {rewrite HPE2. apply Permutation_rel_assoc_swap. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
      exists (S k). eapply pfn_forall. eassumption. 2: {rewrite HPE1, HPE3. apply Permutation_rel_assoc_swap. }
      eapply pfn_perm_rel. reflexivity. apply Permutation_rel_assoc_swap. assumption.
    - admit.
  ...
  Lemma cut_admissibility_other_bang:
    forall p x z n m
      (IHY : ∀ y : nat, y < z → Acc lt y)
      (IHWF : ∀ y : nat,
          y < z
               ...
               ∧ ((∀ n' m' : nat,
                 ...
                 ∧ ((∀ n' m' : nat,
                    ...
                    → cut_admissibility_ques n m
                        (t_base p (b_other x))))
      (Heqo : z = n + m)
      (HC: forall n m, cut_admissibility_norm n m (t_base p (b_other x))),
  cut_admissibility_bang n m (t_base p (b_other x)).
  Proof.
    intros. unfold cut_admissibility_bang, cut_admissibility_t.
    intros c G1 G2 D1 D2 HPD1 HPD2 HPG1 HPG2 HG1 HG2. listSimpl. 
    destruct HG2; subst.
    ...
    - rewrite HPG2 in H0. apply Permutation_rel_split_last in H0.
      ...
      + 
        ...
        assert (HM1: n + n0 < n + S n0) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). specialize (IHWF2 HC). unfold_admissibilityH IHWF2.
        specialize (IHWF2 c G1 G' D1 (D ++ [dual (t_base p (b_other x))])); listSimpl.
        assert (HPD'1: D ++ [dual (t_base p (b_other x))] ≡[P] D ++ [dual (t_base p (b_other x))]) by reflexivity.
        specialize (IHWF2 HPD1 HPD'1 HPG1 HPG2 HG1 HG2) as (k & IHWF2).
        unfold_admissibilityH HC. specialize (HC n k c G1 D1 [] (D ++ [dual (t_base p (b_other x))]) D); listSimpl.
        specialize (HC HPD1 HPD'1 HPG1 HPG1 HG1 IHWF2) as (k2 & HC).
        eexists; eauto.
      + assert (HM1 : n + n0 < n + S n0) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF2 HC); unfold_admissibilityH IHWF2.
    ...
    - assert (HM1: n + n0 < n + S n0) by lia.
      ...
      assert (HPD'1 : D0 ≡[P] D0) by reflexivity.
      assert (HPG'1 : G1 ++ [t] ≡[P] G1 ++ [t]) by reflexivity.
      assert (HPG'2 : G ++ [t] ≡[P] (G1 ++ [t]) ++ [dual (t_base p (b_other x))]) by (rewrite HPG2; apply Permutation_rel_assoc_swap).
      assert (HG'1 : n ⊣ c, (G1 ++ [t]), D1 ⊢pf).
      {eapply pfn_weakening. reflexivity. pfn_wf_ctx_solver. assumption. }
      specialize (IHWF2 HPD1 HPD'1 HPG'1 HPG'2 HG'1 HG2) as (k & IHWF2).
      exists (S k). eapply pfn_bang. 2: {eassumption. } assumption.
  ...
  Lemma cut_admissibility_other_ques:
    forall p x z n m
      (IHY : ∀ y : nat, y < z → Acc lt y)
      (IHWF : ∀ y : nat,
          y < z
      ...
      (Heqo : z = n + m)
      (HC: forall n m, cut_admissibility_norm n m (t_base p (b_other x))),
  cut_admissibility_ques n m (t_base p (b_other x)).
  Proof.
    intros. unfold cut_admissibility_ques, cut_admissibility_t.
    intros c G1 G2 D1 D2 HPD1 HPD2 HPG1 HPG2 HG1 HG2. listSimpl. 
    destruct HG1; subst.
    ...
    - rewrite HPG1 in H0. apply Permutation_rel_split_last in H0.
      ...
      + 
        ...
        assert (HM1: n + m < S n + m) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). specialize (IHWF3 HC). unfold_admissibilityH IHWF3.
        specialize (IHWF3 c G' G2 (D ++ [t_base p (b_other x)]) D2); listSimpl.
        assert (HPD'1: D ++ [t_base p (b_other x)] ≡[P] D ++ [t_base p (b_other x)]) by reflexivity.
        specialize (IHWF3 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF3).
        unfold_admissibilityH HC. specialize (HC k m c G2 (D ++ [t_base p (b_other x)]) D D2 []); listSimpl.
        specialize (HC HPD'1 HPD2 HPG2 HPG2 IHWF3 HG2) as (k2 & HC).
        eexists; eauto.
      + assert (HM1 : n + m < S n + m) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF3 HC); unfold_admissibilityH IHWF3.
    ...
    - assert (HM1: n + m < S n + m) by lia.
      ...
      assert (HPD'1 : D1 ≡[P] D1) by reflexivity.
      assert (HPG'1 : G2 ++ [t] ≡[P] G2 ++ [t]) by reflexivity.
      assert (HPG'2 : G ++ [t] ≡[P] (G2 ++ [t]) ++ [t_base p (b_other x)]) by (rewrite HPG1; apply Permutation_rel_assoc_swap).
      assert (HG'1 : m ⊣ c, (G2 ++ [t]), D2 ⊢pf).
      {eapply pfn_weakening. reflexivity. pfn_wf_ctx_solver. assumption. }
      specialize (IHWF3 HPD'1 HPD2 HPG'2 HPG'1 HG1 HG'1) as (k & IHWF3).
      exists (S k). eapply pfn_bang. 2: {eassumption. } assumption.
  ...
  Lemma cut_admissibility_other :
    forall p x n m,
      cut_admissibility_norm n m (t_base p (b_other x))
      ∧ ((∀ n' m' : nat, cut_admissibility_norm n' m' (t_base p (b_other x)))
         → cut_admissibility_bang n m (t_base p (b_other x)))
      ∧ ((∀ n' m' : nat, cut_admissibility_norm n' m' (t_base p (b_other x)))
         → cut_admissibility_ques n m (t_base p (b_other x))).
  Proof.
    ...
    induction (lt_wf o); intros.
    repeat split.
    - intros. eapply cut_admissibility_other_norm; eassumption.
    - intros. eapply cut_admissibility_other_bang; eassumption.
    - intros. eapply cut_admissibility_other_ques; eassumption.
  Qed.

Lemma Permutation_rel_cons_inv_r: ∀ {a} {l1 l2},  l1 ++ [a] ≡[ P] l2 ++ [a] → l1 ≡[ P] l2.
Proof.
  intros.
  assert (a :: l1 ≡[P] a:: l2). {convertTactics.convert_multisetperm. permutation_solver. }
  eapply Permutation_rel_cons_inv; eauto. apply PermConvertible_P.
Qed.
  
  Lemma cut_admissibility_one_norm:
    forall z n m
      (IHY : ∀ y : nat, y < z → Acc lt y)
      (IHWF : ∀ y : nat,
          y < z
          → ∀ n m : nat,
            y = n + m
            → cut_admissibility_norm n m [1]
              ∧ ((∀ n' m' : nat,
                     cut_admissibility_norm n' m' [1])
                 → cut_admissibility_bang n m [1])
              ∧ ((∀ n' m' : nat,
                     cut_admissibility_norm n' m' [1])
                 → cut_admissibility_ques n m [1]))
      (Heqo : z = n + m),
      cut_admissibility_norm n m [1].
  Proof.
    intros. unfold cut_admissibility_norm, cut_admissibility_t.
    ...
    - subst. assert (HM1: n + m < S n + m) by lia.
      specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G' (D ++ [t]) (D1' ++ [t]) D2 D2').
      assert (HPD'1: D ++ [t] ≡[P] (D1' ++ [t]) ++ [[1]]). {rewrite HPD1. apply Permutation_rel_assoc_swap. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2).
      destruct IHWF1 as (k & IHWF1).
      exists (S k). eapply pfn_absorb. eassumption. eassumption.
      eapply pfn_perm_rel. reflexivity. eapply Permutation_rel_assoc_swap. assumption.
    - symmetry in HPD1. apply Permutation_rel_singleton_nil in HPD1 as (_ & Hcontra). discriminate.
    - rewrite HPD1 in H. apply Permutation_rel_cons_inv_r in H. rename H into HPE2.
      destruct HG2.
      * eapply cut_admissibility'_aux_id2. eassumption. rewrite dual_involutive. eassumption.
        eapply pfn_one. eassumption. rewrite <- HPE2. assumption.
      * assert (HM1: S n + n0 < S n + S n0) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G' (D' ++ [[1]]) D1' (D0 ++ [t]) (D2' ++ [t])).
        assert (HPD'1: D' ++ [[1]] ≡[P] D1' ++ [[1]]). {rewrite HPE2. reflexivity. }
        assert (HPD'2: D0 ++ [t] ≡[P] (D2' ++ [t]) ++ [dual [1]]). {rewrite HPD2. apply Permutation_rel_assoc_swap. }
        eapply pfn_one in HG1. 2: {reflexivity. }
        specialize (IHWF1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
        exists (S k). eapply pfn_absorb. eassumption. eassumption. rewrite <- app_assoc. assumption.
      * symmetry in HPD2. apply Permutation_rel_singleton_nil in HPD2 as (-> & _).
        exists n. eapply pfn_perm_rel. reflexivity. rewrite app_nil_r. symmetry. apply HPE2. assumption.
      * rewrite HPD2 in H. apply Permutation_rel_split_last in H.
        destruct H as [[HPF1 HPF2] | [F1 [F2 [HPF1 [HPF2 HPF3]]]]]; try discriminate.
        assert (HM1: S n + n0 < S n + S n0) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G (D' ++ [[1]]) D1' D'0 F2).
        assert (HPD'1: D' ++ [[1]] ≡[P] D1' ++ [[1]]). {rewrite HPE2. reflexivity. }
        eapply pfn_one in HG1. 2: {reflexivity. }
        specialize (IHWF1 HPD'1 HPF2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
        exists (S k). eapply pfn_one. 2: {rewrite HPF1, HPF3, app_assoc. reflexivity. } assumption.
      * rewrite HPD2 in H. apply Permutation_rel_split_last in H.
        ...
        assert (HM1: S n + n0 < S n + S n0) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G (D' ++ [[1]]) D1' (D'0 ++ [t] ++ [u]) (F1 ++ [t] ++ [u])).
        assert (HPD'1: D' ++ [[1]] ≡[P] D1' ++ [[1]]). {rewrite HPE2. reflexivity. }
        assert (HPD'2: D'0 ++ [t] ++ [u] ≡[P] (F1 ++ [t] ++ [u]) ++ [dual ([1])]). {rewrite HPF2, <- HPF3. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. }
        eapply pfn_one in HG1. 2: {reflexivity. }
        specialize (IHWF1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
        exists (S k). eapply pfn_tensor.
        -- rewrite app_assoc in IHWF1. apply IHWF1.
      ...
      * rewrite HPD2, app_assoc in H. apply Permutation_rel_split_last in H.
        ...
        -- assert (HM1: S n + n1 < S n + S (Nat.max n1 n2)) by lia.
           specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
           eapply pfn_one in HG1. 2: {reflexivity. }
           specialize (IHWF1 c G (D' ++ [[1]]) D1' (D1 ++ [t]) (I1 ++ [t])).
           assert (HPD'1: D' ++ [[1]] ≡[P] D1' ++ [[1]]). {rewrite HPE2. reflexivity. }
           assert (HPD'2: D1 ++ [t] ≡[P] (I1 ++ [t]) ++ [dual ([1])]). {rewrite HPI1. apply Permutation_rel_assoc_swap. }
           specialize (IHWF1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2_1) as (k & IHWF1).
           exists (S (Nat.max k n2)). eapply pfn_par.
           3: {assert (D1' ++ D2' ≡[P] (D1' ++ I1) ++ D2 ++ [t_par t u]). {rewrite HPF1 , HPF3, <- HPI2. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. } eassumption. }
           ++ rewrite <- app_assoc. assumption.
           ++ assumption.
        -- assert (HM1: S n + n2 < S n + S (Nat.max n1 n2)) by lia.
           specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
           specialize (IHWF1 c G (D' ++ [[1]]) D1' (D2 ++ [u]) (I2 ++ [u])).
           assert (HPD'1: D' ++ [[1]] ≡[P] D1' ++ [[1]]). {rewrite HPE2. reflexivity. }
           assert (HPD'2: D2 ++ [u] ≡[P] (I2 ++ [u]) ++ [dual ([1])]). {rewrite HPI1. apply Permutation_rel_assoc_swap. }
           eapply pfn_one in HG1. 2: {reflexivity. }
           specialize (IHWF1 HPD'1 HPD'2 HPG1 HPG2 HG1 HG2_2) as (k & IHWF1).
           exists (S (Nat.max n1 k)). eapply pfn_par.
           3: {assert (D1' ++ D2' ≡[P] D1 ++ (D1' ++ I2) ++ [t_par t u]). {rewrite HPF1, HPF3, <- HPI2. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. } eassumption. }
           ++ assumption.
           ++ rewrite <- app_assoc. apply IHWF1.
      * rewrite HPD2 in H. apply Permutation_rel_split_last in H.
        destruct H as [[HPF1 HPF2] | [F1 [F2 [HPF1 [HPF2 HPF3]]]]]; try discriminate.
        assert (HM1: S n + n0 < S n + S n0) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c (G ++ [t]) (D' ++ [[1]]) D1' D1 F2); listSimpl.
        assert (HPD'1: D' ++ [[1]] ≡[P] D1' ++ [[1]]). {rewrite HPE2. reflexivity. }
        eapply pfn_one in HG1. 2: {reflexivity. }
        assert (HG'1: S n ⊣ c, G ++ [t], D' ++ [[1]] ⊢pf). {eapply pfn_weakening. reflexivity. pfn_wf_ctx_solver. assumption. }
        assert (HPG'1: G ++ [t] ≡[P] G ++ [t]) by reflexivity.
        specialize (IHWF1 HPD'1 HPF2 HPG'1 HPG'1 HG'1 HG2) as (k & IHWF1).
        exists (S k). eapply pfn_bang. 2: {rewrite HPF1, HPF3, app_assoc. reflexivity. } assumption.
      * symmetry in HPD2. apply Permutation_rel_singleton_nil in HPD2 as (-> & HPD2). discriminate.
      * rewrite HPD2 in H0. apply Permutation_rel_split_last in H0.
        destruct H0 as [[HPF1 HPF2] |[F1 [F2 [HPF1 [HPF2 HPF3]]]]]; try discriminate.
        assert (HM1: S n + n0 < S n + S n0) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G D D1' (D1 ++ [typ_subst c u t]) (F2 ++ [typ_subst c u t])); listSimpl.
        assert (HPD'2: D1 ++ [typ_subst c u t] ≡[P] (F2 ++ [typ_subst c u t]) ++ [dual ([1])]). {rewrite HPF2. apply Permutation_rel_assoc_swap. }
        eapply pfn_one in HG1. 2: {rewrite <- HPE2. apply HPD1. }
        specialize (IHWF1 HPD1 HPD'2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
        exists (S k). eapply pfn_forall. eassumption. rewrite app_assoc in IHWF1. eassumption. rewrite HPF1, HPF3, app_assoc. reflexivity.
      * admit.
    - rewrite HPD1 in H. apply Permutation_rel_split_last in H.
      ...
      specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G (D' ++ [t] ++ [u]) (E1 ++ [t] ++ [u]) D2 D2').
      assert (HPD'1: D' ++ [t] ++ [u] ≡[P] (E1 ++ [t] ++ [u] ) ++ [[1]]). {rewrite HPE2, <- HPE3. clear_permrel. convertTactics.convert_multisetperm. permutation_solver. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
      exists (S k). eapply pfn_tensor. 2: {rewrite HPE1. apply Permutation_rel_assoc_swap. }
      eapply pfn_perm_rel. reflexivity. assert ((E1 ++ [t] ++ [u]) ++ D2' ≡[P] ((E1 ++ D2') ++ [t] ++ [u])). {clear_permrel. convertTactics.convert_multisetperm. permutation_solver. } eassumption. assumption.
    - rewrite HPD1, app_assoc in H. apply Permutation_rel_split_last in H.
      ...
      + subst. assert (HM1: n1 + m < S (Nat.max n1 n2) + m) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G (D1 ++ [t]) (F1 ++ [t]) D2 D2').
        assert (HPD'1: D1 ++ [t] ≡[P]  (F1 ++ [t]) ++ [[1]]). {rewrite HPF1. apply Permutation_rel_assoc_swap. }
        specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1_1 HG2) as (k & IHWF1).
        exists (S (Nat.max k n2)). eapply pfn_par.
        * 
          eapply pfn_perm_rel. reflexivity. assert ((F1 ++ [t]) ++ D2' ≡[P] (F1 ++ D2') ++ [t]) by apply Permutation_rel_assoc_swap. apply H. assumption.
      ...
      + subst. assert (HM1: n2 + m < S (Nat.max n1 n2) + m) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). unfold_admissibilityH IHWF1.
        specialize (IHWF1 c G (D0 ++ [u]) (F2 ++ [u]) D2 D2').
        assert (HPD'1: D0 ++ [u] ≡[P] (F2 ++ [u]) ++ [[1]]). {rewrite HPF1. apply Permutation_rel_assoc_swap. }
        specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1_2 HG2) as (k & IHWF1).
        exists (S (Nat.max n1 k)). eapply pfn_par.
        * 
          eapply HG1_1.
    ...
    - rewrite HPD1 in H0. apply Permutation_rel_split_last in H0. 
      ...
      specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IWHF2 & IHWF3). unfold_admissibilityH IHWF1.
      specialize (IHWF1 c G (D1 ++ [typ_subst c u t]) (E2 ++ [typ_subst c u t]) D2 D2').
      assert (HPD'1: D1 ++ [typ_subst c u t] ≡[P] (E2 ++ [typ_subst c u t]) ++ [[1]]). {rewrite HPE2. apply Permutation_rel_assoc_swap. }
      specialize (IHWF1 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF1).
      exists (S k). eapply pfn_forall. eassumption. 2: {rewrite HPE1, HPE3. apply Permutation_rel_assoc_swap. }
      eapply pfn_perm_rel. reflexivity. apply Permutation_rel_assoc_swap. assumption.
    - admit.
  ...
  Lemma cut_admissibility_one_bang:
    forall z n m
      (IHY : ∀ y : nat, y < z → Acc lt y)
      (IHWF : ∀ y : nat,
          y < z
             ...
             → cut_admissibility_norm n m [1]
               ∧ ((∀ n' m' : nat,
                 ...
                 ∧ ((∀ n' m' : nat,
                    ...
                    → cut_admissibility_ques n m [1]))
      (Heqo : z = n + m)
      (HC : ∀ n' m' : nat, cut_admissibility_norm n' m' [1]),
      cut_admissibility_bang n m [1].
  Proof.
    intros. unfold cut_admissibility_bang, cut_admissibility_t.
    intros c G1 G2 D1 D2 HPD1 HPD2 HPG1 HPG2 HG1 HG2. listSimpl. 
    ...
    - rewrite HPG2 in H0. apply Permutation_rel_split_last in H0.
      ...
      + 
        ...
        assert (HM1: n + n0 < n + S n0) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). specialize (IHWF2 HC). unfold_admissibilityH IHWF2.
        specialize (IHWF2 c G1 G' D1 (D ++ [dual [1]])); listSimpl.
        assert (HPD'1: D ++ [dual [1]] ≡[P] D ++ [dual [1]]) by reflexivity.
        specialize (IHWF2 HPD1 HPD'1 HPG1 HPG2 HG1 HG2) as (k & IHWF2).
        unfold_admissibilityH HC. specialize (HC n k c G1 D1 [] (D ++ [dual [1]]) D); listSimpl.
        specialize (HC HPD1 HPD'1 HPG1 HPG1 HG1 IHWF2) as (k2 & HC).
        eexists; eauto.
      + assert (HM1 : n + n0 < n + S n0) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF2 HC); unfold_admissibilityH IHWF2.
    ...
    - assert (HM1: n + n0 < n + S n0) by lia.
      ...
      assert (HPD'1 : D0 ≡[P] D0) by reflexivity.
      assert (HPG'1 : G1 ++ [t] ≡[P] G1 ++ [t]) by reflexivity.
      assert (HPG'2 : G ++ [t] ≡[P] (G1 ++ [t]) ++ [dual [1]]) by (rewrite HPG2; apply Permutation_rel_assoc_swap).
      assert (HG'1 : n ⊣ c, (G1 ++ [t]), D1 ⊢pf).
      {eapply pfn_weakening. reflexivity. pfn_wf_ctx_solver. assumption. }
      specialize (IHWF2 HPD1 HPD'1 HPG'1 HPG'2 HG'1 HG2) as (k & IHWF2).
      exists (S k). eapply pfn_bang. 2: {eassumption. } assumption.
  ...
  Lemma cut_admissibility_one_ques:
    forall z n m
      (IHY : ∀ y : nat, y < z → Acc lt y)
      (IHWF : ∀ y : nat,
          y < z
  ...
  Proof.
    ...
    - rewrite HPG1 in H0. apply Permutation_rel_split_last in H0.
      ...
      + 
        ...
        assert (HM1: n + m < S n + m) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3). specialize (IHWF3 HC). unfold_admissibilityH IHWF3.
        specialize (IHWF3 c G' G2 (D ++ [[1]]) D2); listSimpl.
        assert (HPD'1: D ++ [[1]] ≡[P] D ++ [[1]]) by reflexivity.
        specialize (IHWF3 HPD'1 HPD2 HPG1 HPG2 HG1 HG2) as (k & IHWF3).
        unfold_admissibilityH HC. specialize (HC k m c G2 (D ++ [[1]]) D D2 []); listSimpl.
        specialize (HC HPD'1 HPD2 HPG2 HPG2 IHWF3 HG2) as (k2 & HC).
        eexists; eauto.
      + assert (HM1 : n + m < S n + m) by lia.
        specialize (IHWF _ HM1 _ _ eq_refl) as (IHWF1 & IHWF2 & IHWF3); specialize (IHWF3 HC); unfold_admissibilityH IHWF3.
    ...
    - assert (HM1: n + m < S n + m) by lia.
      ...
      assert (HPD'1 : D1 ≡[P] D1) by reflexivity.
      assert (HPG'1 : G2 ++ [t] ≡[P] G2 ++ [t]) by reflexivity.
      assert (HPG'2 : G ++ [t] ≡[P] (G2 ++ [t]) ++ [[1]]) by (rewrite HPG1; apply Permutation_rel_assoc_swap).
      assert (HG'1 : m ⊣ c, (G2 ++ [t]), D2 ⊢pf).
      {eapply pfn_weakening. reflexivity. pfn_wf_ctx_solver. assumption. }
      specialize (IHWF3 HPD'1 HPD2 HPG'2 HPG'1 HG1 HG'1) as (k & IHWF3).
      exists (S k). eapply pfn_bang. 2: {eassumption. } assumption.
  ...
  Lemma cut_admissibility_one:
    ∀ n m : nat,
      cut_admissibility_norm n m [1]
      ∧ ((∀ n' m' : nat, cut_admissibility_norm n' m' [1]) → cut_admissibility_bang n m [1])
      ∧ ((∀ n' m' : nat, cut_admissibility_norm n' m' [1]) → cut_admissibility_ques n m [1]).
  Proof.
    intros n m.
    remember (n + m) as o.
    revert n m Heqo.
    ...
    - intros. eapply cut_admissibility_one_norm; eassumption.
    - intros. eapply cut_admissibility_one_bang; eassumption.
    - intros. eapply cut_admissibility_one_ques; eassumption.
  Qed.
  
  Lemma cut_admissibility_base :
    forall p b n m,
      cut_admissibility_norm n m (t_base p b)
      ∧ ((∀ n' m' : nat,
             cut_admissibility_norm n' m' (t_base p b))
         → cut_admissibility_bang n m (t_base p b))
      ∧ ((∀ n' m' : nat,
             cut_admissibility_norm n' m' (t_base p b))
         → cut_admissibility_ques n m (t_base p b)).
  Proof.
    intros p b. revert p.
    induction b.
    - destruct p.
      + intros. apply cut_admissibility_one; assumption.
      + apply (cut_admissibility_dual ([⊥])).
        apply cut_admissibility_one.
    - intros. eapply cut_admissibility_other.
  Qed.
